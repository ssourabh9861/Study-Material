...
Cover at minimum (expand well beyond): the CAP theorem stated precisely (and the common misreadings); why "partition tolerance" is not optional in practice; PACELC as the better model (Else: Latency vs Consistency); how real systems (Dynamo/Cassandra, Spanner, ZooKeeper, MongoDB) map onto CP/AP and PACELC; what "consistency" in CAP actually means vs other uses of the word; concrete design implications for choosing AP vs CP for money vs non-money data.
