...
Cover at minimum: the full spectrum — strict/linearizable, sequential, causal, read-your-writes, monotonic reads/writes, bounded-staleness, eventual; precise definitions with example histories; the difference between consistency (CAP/replication) and isolation (transactions); CRDTs as a way to get convergence; which databases offer which model and how you configure it; client-centric vs data-centric models.
