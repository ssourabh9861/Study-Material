...
Cover at minimum: failure detection fundamentals (heartbeats, timeouts, the accuracy/completeness tradeoff); phi-accrual failure detector; gossip protocols and SWIM membership; how clusters agree on who is alive (ZooKeeper sessions, Kafka, Cassandra gossip); split-brain and fencing tokens; the difference between "crashed" and "slow"; cascading failures from aggressive detection.
