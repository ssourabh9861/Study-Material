...
**Subtopic:** Transactional Outbox & CDC

Cover at minimum: the dual-write problem (why writing to DB + queue separately is broken); the transactional outbox pattern (outbox table written in the same DB tx, relay publishes); Change Data Capture explained from scratch (log/WAL tailing); Debezium + Kafka Connect end-to-end; the inbox pattern and idempotent consumers; ordering and exactly-once concerns; polling-publisher vs log-tailing; schema for an outbox table + relay code.
