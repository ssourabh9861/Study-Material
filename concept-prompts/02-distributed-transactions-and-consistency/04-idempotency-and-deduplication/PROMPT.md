...
Cover at minimum: what idempotency means and why it's the backbone of reliable distributed systems; idempotency keys (generation, storage, TTL, scope); dedup stores (Redis/DB unique constraints/CAS); idempotent consumers under at-least-once delivery; exactly-once *effects* vs exactly-once delivery; natural vs synthetic idempotency; making non-idempotent operations idempotent; worked Java examples for an idempotent payment endpoint and an idempotent message consumer.
