...
Cover at minimum: monolith, modular monolith, microservices, event-driven, serverless — what each is and its real tradeoffs; Conway's law; the true costs of microservices (network, data, ops, distributed transactions); when a monolith is the senior choice; decomposition strategies (DDD bounded contexts); migration patterns (strangler fig); how to decide.
