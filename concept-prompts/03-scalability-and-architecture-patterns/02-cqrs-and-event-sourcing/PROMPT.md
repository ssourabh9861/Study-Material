...
Cover at minimum: command/query responsibility segregation explained from scratch; event sourcing (the event log as source of truth, rebuilding state, snapshots); how CQRS+ES combine; benefits (audit, temporal queries, scaling reads) and heavy costs (complexity, eventual consistency, versioning events); when NOT to use them; projections/read models; worked example; relationship to Kafka/changelog topics.
