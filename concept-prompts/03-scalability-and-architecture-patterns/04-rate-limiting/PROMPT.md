...
Cover at minimum: why rate limit (protection, fairness, cost); algorithms in depth — fixed window, sliding window log, sliding window counter, token bucket, leaky bucket (with code); distributed rate limiting (Redis-based, with race conditions and Lua scripts); per-user/per-tenant/global limits; rate limiting vs throttling vs load shedding; response design (429, Retry-After, headers); where to enforce (gateway vs service).
