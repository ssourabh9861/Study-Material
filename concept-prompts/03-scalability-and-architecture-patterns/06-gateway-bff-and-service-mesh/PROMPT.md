...
**Subtopic:** API Gateway, BFF & Service Mesh

Cover at minimum: API gateway responsibilities (auth, routing, throttling, aggregation, TLS termination) and what should NOT live there; Backend-for-Frontend pattern; service mesh (sidecar proxy, Envoy/Istio/Linkerd) — what it actually solves (mTLS, retries, observability, traffic shifting); gateway vs mesh vs library; data plane vs control plane; performance overhead; when each is overkill.
