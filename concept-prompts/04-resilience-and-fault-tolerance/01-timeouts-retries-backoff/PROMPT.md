...
**Subtopic:** Timeouts, Retries, Backoff & Jitter

Cover at minimum: why every remote call needs a timeout (connect vs read vs total); choosing timeout values from latency percentiles; retries and the danger of naive retries (retry storms, metastable failures); exponential backoff and the necessity of jitter (full/equal/decorrelated — explain each); retry budgets and circuit-breaker interaction; idempotency as a precondition for safe retries; deadline propagation across hops; Java examples (Resilience4j, gRPC deadlines).
