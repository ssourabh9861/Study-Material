...
Cover at minimum: the bulkhead metaphor and goal (contain failure, prevent resource exhaustion from spreading); thread-pool isolation vs semaphore isolation (tradeoffs); connection-pool partitioning per dependency; isolating tenants/priorities; how a single slow dependency exhausts a shared pool (with a concrete trace); Resilience4j Bulkhead; sizing pools (Little's Law); the cost of isolation.
