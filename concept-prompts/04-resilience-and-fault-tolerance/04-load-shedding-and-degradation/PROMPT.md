...
Cover at minimum: load shedding vs rate limiting vs backpressure; prioritized admission control (drop low-priority first); detecting overload (queue depth, latency, CPU); graceful degradation patterns (serve stale, reduce fidelity, disable non-critical features); brownout; the difference between shedding load and shedding the right load; LIFO vs FIFO under overload; worked examples and real incidents (e.g., retry-amplified outages).
