...
Cover at minimum: the thundering-herd problem (many clients hit the same resource at once); cache stampede/dogpile on expiry; mitigations — request coalescing/single-flight, per-key locks, probabilistic early expiration, staggered TTLs/jitter, serve-stale-while-revalidate; herd on cold start and on cache flush; retry synchronization; worked Java/Redis examples; relation to backoff jitter.
