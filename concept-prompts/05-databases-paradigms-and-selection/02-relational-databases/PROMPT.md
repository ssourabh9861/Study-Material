...
Cover at minimum: the relational model and SQL essentials a senior must know cold (joins, indexes, query planning, EXPLAIN); B-tree indexes and how the planner uses them; transactions and locking in Postgres/MySQL (InnoDB); normalization vs denormalization; connection pooling; common performance pitfalls (N+1, missing indexes, lock contention, long transactions); when relational is the right default; partitioning basics.
