...
**Subtopic:** Wide-Column Stores

Cover at minimum: the wide-column/BigTable model (row key, column families, sparse columns) explained from scratch; HBase and Cassandra internals (and how they differ — HBase CP+HDFS, Cassandra AP+gossip); LSM-tree storage, memtables, SSTables, compaction; designing the row/partition key for access patterns and to avoid hotspots; tombstones and read amplification; when wide-column wins (huge write throughput, time-series, wide sparse data); query limitations.
