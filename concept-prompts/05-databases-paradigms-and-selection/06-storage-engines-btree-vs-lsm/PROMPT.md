...
**Subtopic:** Storage Engines — B-Tree vs LSM-Tree

Cover at minimum: how B-trees work (pages, balancing, in-place updates, write-ahead log) vs LSM-trees (memtable, SSTables, leveled/size-tiered compaction); read/write/space amplification compared; why LSM is write-optimized and B-tree read-optimized; bloom filters; compaction strategies and their stalls; which databases use which (Postgres/MySQL=B-tree, Cassandra/RocksDB/HBase/LevelDB=LSM); choosing based on workload.
