...
**Subtopic:** Partitioning & Sharding

Cover at minimum: vertical vs horizontal partitioning; functional decomposition vs sharding; sharding strategies — range, hash, directory/lookup, geo (with tradeoffs); choosing a shard key (cardinality, even distribution, query alignment); hot shards/celebrity problem and mitigations; cross-shard queries, joins, and transactions; rebalancing; application-level vs middleware (Vitess) vs native sharding; worked examples.
