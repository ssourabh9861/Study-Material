...
**Subtopic:** Multi-Region & Geo-Partitioning

Cover at minimum: why go multi-region (latency, availability, data residency); geo-partitioning data by region; active-active vs active-passive at the data layer; cross-region replication and the latency-vs-consistency wall; conflict resolution; data residency/compliance (GDPR); follow-the-sun and read-local/write-global patterns; how Spanner/CockroachDB/DynamoDB Global Tables approach it; the failure and reconciliation story.
