...
Cover at minimum: Redis single-threaded event-loop model (and why it's fast); core data structures and their use cases (string, hash, list, set, sorted set, bitmap, HyperLogLog, streams, geo); persistence (RDB snapshots vs AOF, hybrid, fsync policy); replication and Redis Sentinel vs Redis Cluster (hash slots, resharding); pipelining, transactions (MULTI/EXEC), Lua scripting; pub/sub and streams; memory model and eviction; common patterns (rate limiter, leaderboard, lock, queue) with code.
