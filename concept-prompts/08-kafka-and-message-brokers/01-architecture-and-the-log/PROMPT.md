...
**Subtopic:** Architecture & The Commit Log

Cover at minimum: the partitioned, append-only commit log model (and how it differs from a traditional queue); topics, partitions, offsets, segments, index files; sequential I/O, page cache, and zero-copy (sendfile) — explain each; brokers, the cluster, and how data is laid out on disk; producer→broker→consumer data flow end to end; why the log model enables replay and high throughput; ordering guarantees (per-partition only).
