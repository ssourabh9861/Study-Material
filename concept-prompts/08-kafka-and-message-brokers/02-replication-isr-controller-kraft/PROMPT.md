...
**Subtopic:** Replication, ISR, Controller & KRaft

Cover at minimum: leader/follower replication; the in-sync replica (ISR) set and how membership changes; the high-watermark and committed messages; min.insync.replicas + acks=all durability contract; the controller's role; **ZooKeeper explained from scratch** (a distributed coordination service: what it is, znodes, watches, why Kafka used it) and why Kafka moved to **KRaft** (Raft-based metadata quorum) — explain Raft briefly too; unclean leader election; under-replicated partitions.
