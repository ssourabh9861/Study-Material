...
Cover at minimum: what triggers a rebalance (membership/subscription/partition changes); eager (stop-the-world) vs incremental cooperative-sticky rebalancing (KIP-429) explained step by step; the rebalance protocol (group coordinator, join/sync); static membership (KIP-345) to avoid rebalances on restart; rebalance storms (cause and cure); the relationship between max.poll.interval.ms and rebalances; partition revocation callbacks and committing offsets safely.
