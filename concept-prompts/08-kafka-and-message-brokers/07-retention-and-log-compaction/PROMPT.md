...
**Subtopic:** Retention & Log Compaction

Cover at minimum: delete retention (time/size) vs compaction cleanup policy; how compaction works (latest value per key, the cleaner, tombstones, delete.retention.ms); compacted topics as changelogs/snapshots (Kafka Streams state stores, CDC); compact+delete; segment roll and cleaning internals; using compaction for event-carried state transfer; gotchas (compaction is async/best-effort); worked examples (config + a changelog topic).
