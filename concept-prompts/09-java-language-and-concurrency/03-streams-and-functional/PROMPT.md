...
**Subtopic:** Streams & Functional Java

Cover at minimum: lambdas and functional interfaces; the Stream pipeline (source, intermediate lazy ops, terminal op); map/filter/reduce/collect; Collectors (groupingBy, toMap, partitioning, joining); statefulness and ordering; parallel streams (when they help/hurt, the common ForkJoinPool, splititerators); Optional done right; method references; performance vs loops; when NOT to use streams; worked examples for real data transformations.
