...
**Subtopic:** The Java Memory Model (JMM)

Cover at minimum: why a memory model exists (compiler/CPU reordering, per-core caches, store buffers); the happens-before relation in full (program order, monitor, volatile, thread start/join, final fields); volatile semantics and what it does/does not guarantee; data races defined; safe publication; final-field freeze semantics; double-checked locking done correctly; false sharing and cache lines; worked examples showing broken vs correct code.
