...
**Concept area:** Java Language & Concurrency
**Subtopic:** Executors & Thread Pools

Cover at minimum: the Executor/ExecutorService/ScheduledExecutorService APIs; ThreadPoolExecutor internals in depth (core/max pool size, keepAlive, the work queue, the handshake of when a new thread vs queue vs reject happens — this trips people up); queue choices (SynchronousQueue, LinkedBlockingQueue, ArrayBlockingQueue) and how they change behavior; rejection policies; Executors factory methods and why newFixedThreadPool/newCachedThreadPool can be dangerous; sizing pools (CPU-bound = N+1, IO-bound via Little's Law / target utilization, with the formula); ForkJoinPool and work-stealing; graceful shutdown (shutdown vs shutdownNow, awaitTermination); naming threads; full Java examples for several use cases.
