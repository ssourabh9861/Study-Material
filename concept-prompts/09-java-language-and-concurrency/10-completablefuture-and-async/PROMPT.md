...
**Subtopic:** CompletableFuture & Async Pipelines

Cover at minimum: Future vs CompletableFuture; composition (thenApply/thenCompose/thenCombine, the async variants and which pool runs them — a major gotcha); exception handling (exceptionally, handle, whenComplete); combining many futures (allOf/anyOf) and collecting results; timeouts (orTimeout, completeOnTimeout); avoiding blocking the common pool; structuring async service calls; backpressure limits; full examples (parallel fan-out/fan-in, sequential dependency, fallback).
