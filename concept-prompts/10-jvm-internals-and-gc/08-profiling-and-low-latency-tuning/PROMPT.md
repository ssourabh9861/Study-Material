...
**Concept area:** JVM Internals & Garbage Collection
**Subtopic:** Profiling & Low-Latency Tuning

Cover at minimum: profiling approaches (sampling vs instrumentation; safepoint bias and why async-profiler avoids it); JFR (Java Flight Recorder) and JDK Mission Control; async-profiler (CPU, alloc, lock, wall-clock flame graphs); finding hot methods, allocation hotspots, lock contention; low-latency techniques (minimize allocation, object reuse/pooling carefully, off-heap, avoiding GC on hot path, warmup, pinning to cores, choosing ZGC); measuring p99/p999; methodology for a latency investigation.
