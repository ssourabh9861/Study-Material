...
**Subtopic:** Spring Data JPA

Cover at minimum: the abstraction stack (Spring Data JPA → JPA → Hibernate → JDBC); repository interfaces (CrudRepository/JpaRepository), derived query methods, @Query (JPQL + native), projections, Specifications, Querydsl; pagination and sorting; the EntityManager and persistence context behind repositories; @Modifying queries; auditing; how repositories are implemented (proxies); common pitfalls (N+1 from lazy in views, derived-query explosion); worked examples.
