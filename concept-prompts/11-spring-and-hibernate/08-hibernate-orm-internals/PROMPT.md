...
**Subtopic:** Hibernate ORM Internals

Cover at minimum: the object-relational impedance mismatch; the Session/EntityManager and the persistence context (first-level cache); entity lifecycle states (transient, managed/persistent, detached, removed) and transitions; automatic dirty checking and flush (flush modes, flush order); the write-behind/action-queue; identity (@Id generation strategies and their batching implications, IDENTITY vs SEQUENCE); save vs persist vs merge vs saveOrUpdate; how a flush translates to SQL; worked examples and surprising flush behavior.
