...
**Subtopic:** Entity Mappings & Relationships

Cover at minimum: @Entity/@Table/@Column basics; relationship mappings (@OneToOne, @OneToMany, @ManyToOne, @ManyToMany) with owning vs inverse side, mappedBy, join columns/tables; cascade types and orphanRemoval; embeddables and composite keys; inheritance strategies (SINGLE_TABLE, JOINED, TABLE_PER_CLASS — tradeoffs); equals/hashCode for entities (the detached-entity trap); bidirectional consistency helpers; worked schema + mapping examples.
