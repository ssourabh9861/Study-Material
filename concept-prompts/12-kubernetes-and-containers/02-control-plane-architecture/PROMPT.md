...
**Subtopic:** Control Plane Architecture

Cover at minimum: the declarative/reconciliation model (desired vs actual state, control loops); control-plane components — kube-apiserver (the only thing that talks to etcd, validation/admission), etcd (the Raft-backed store — explain etcd/Raft briefly), scheduler, controller-manager; node components — kubelet, kube-proxy, container runtime; how a `kubectl apply` flows through the system end to end; admission controllers and webhooks; the watch/informer mechanism; operators/CRDs as extension.
