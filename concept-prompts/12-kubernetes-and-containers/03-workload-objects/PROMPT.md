...
**Subtopic:** Workload Objects

Cover at minimum: Pod (the atom, multi-container/sidecar, init containers, pause container); ReplicaSet; Deployment (rollouts, revision history); StatefulSet (stable identity, ordered deploy, per-pod storage — when you need it); DaemonSet; Job/CronJob; the controller pattern behind each; labels/selectors/annotations; ownerReferences and garbage collection; choosing the right workload object; worked YAML examples.
