...
**Subtopic:** Rollouts & Probes

Cover at minimum: Deployment rollout strategies (RollingUpdate maxSurge/maxUnavailable, Recreate); canary/blue-green on K8s (and tools — Argo Rollouts, Flagger); liveness vs readiness vs startup probes (what each does, the difference that matters — readiness gates traffic, liveness restarts) and how to set them correctly (and the danger of liveness-induced restart loops); graceful shutdown (preStop hook, terminationGracePeriod, SIGTERM handling, draining); PodDisruptionBudgets; worked examples.
