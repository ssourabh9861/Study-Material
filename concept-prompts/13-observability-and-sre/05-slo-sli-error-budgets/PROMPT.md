...
**Subtopic:** SLI, SLO, SLA & Error Budgets

Cover at minimum: precise definitions (SLI = a measured ratio, SLO = target, SLA = contract with consequences); choosing good SLIs (the four golden signals: latency, traffic, errors, saturation; RED for services, USE for resources); defining SLOs (availability and latency, percentile choice, measurement window); error budgets and budget policy (freeze releases when burned); multi-window multi-burn-rate alerting (fast + slow); how SLOs drive engineering prioritization; worked SLO definitions.
