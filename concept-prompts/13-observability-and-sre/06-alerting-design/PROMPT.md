...
**Subtopic:** Alerting Design

Cover at minimum: alert on symptoms (user-visible) not causes; the properties of a good alert (actionable, urgent, novel); page vs ticket vs dashboard; SLO/error-budget burn-rate alerting (multi-window) vs static thresholds; alert fatigue and how to fight it (deduplication, grouping, inhibition, tuning); runbooks and the on-call experience; routing/escalation (PagerDuty/Opsgenie); the "every alert must require action" rule; worked Prometheus Alertmanager examples.
