...
**Subtopic:** Grafana & Dashboards

Cover at minimum: what makes a good dashboard (answers a question, tells a story, top-down from SLO to detail); the USE and RED dashboard layouts; panel types and when to use them; variables/templating for reusable dashboards; avoiding the "wall of graphs" anti-pattern; linking dashboards to traces (exemplars) and logs; dashboards-as-code (provisioning, JSON, Grafana Terraform); alerting in Grafana vs Prometheus; worked dashboard design for a service.
