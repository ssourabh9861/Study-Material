...
Cover at minimum: the incident lifecycle (detect → triage → mitigate → resolve → learn); severity levels and what they trigger; incident command (roles: IC, comms, ops) for big incidents; mitigation-first mindset (roll back, failover, shed load before root-causing); MTTD/MTTR and how observability reduces them; blameless postmortems (timeline, root cause vs contributing factors, the 5 whys, action items with owners); error-budget linkage; building a learning culture; a postmortem template.
