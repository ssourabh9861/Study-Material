...
**Subtopic:** Secrets Management & Encryption

Cover at minimum: encryption at rest vs in transit vs in use; symmetric vs asymmetric crypto and where each is used; key management (KMS, envelope encryption with DEK/KEK, key rotation, HSMs); secrets management with Vault (dynamic secrets, leases, transit engine) and cloud secret managers; never hardcode/commit secrets (and detection — git-secrets, scanning); tokenization vs encryption; hashing passwords (bcrypt/scrypt/Argon2, salts, peppers — never plain SHA); TLS cert management; worked examples.
