...
Cover at minimum: the problem GraphQL solves (over/under-fetching, client-driven queries); schema/types/resolvers; queries/mutations/subscriptions; the N+1 resolver problem and DataLoader batching; pagination (Relay cursor connections); caching challenges (no URL-based HTTP cache, persisted queries); security concerns (query depth/complexity limiting, introspection in prod); federation for microservices; when GraphQL is the wrong choice; worked schema + resolver example.
