...
**Subtopic:** Versioning & Compatibility

Cover at minimum: backward vs forward compatibility (precise definitions); API versioning strategies (URI path, header, media-type/content negotiation, query param) with tradeoffs; the principle of evolving without versioning where possible (additive changes only); breaking vs non-breaking changes catalog; Protobuf/Avro schema evolution rules (field numbers, reserved, defaults); deprecation policy and sunset headers; consumer-driven contract testing (Pact); worked examples.
