...
**Subtopic:** Pagination & Bulk APIs

Cover at minimum: offset/limit pagination and why it breaks at scale (slow deep offsets, inconsistency under inserts); cursor/keyset pagination (how it works, opaque cursors, stable sort keys); page-token design; total counts (and why to avoid them); bulk/batch endpoints (request shape, partial success, limits); streaming large result sets; long-running operations (async job + status polling / callbacks); worked examples (SQL for keyset, API contracts).
