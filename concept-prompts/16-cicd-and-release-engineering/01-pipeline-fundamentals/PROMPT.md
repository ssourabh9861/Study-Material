...
**Concept area:** CI/CD & Release Engineering
**Subtopic:** Pipeline Fundamentals

Cover at minimum: CI vs CD (delivery vs deployment); the canonical pipeline stages (checkout → build → unit test → static analysis/scan → package artifact → integration test → deploy → smoke test → promote); build reproducibility and hermetic/deterministic builds; caching and incremental builds (and the monorepo angle — Bazel); artifact immutability and promotion across environments (build once, deploy many); pipeline-as-code (GitHub Actions/GitLab CI/Jenkins); fast feedback and pipeline speed; worked pipeline example.
