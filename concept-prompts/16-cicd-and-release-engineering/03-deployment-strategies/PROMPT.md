...
Cover at minimum: recreate, rolling, blue-green, canary, and shadow/dark-launch — each explained with mechanics, rollback story, cost, and risk profile; automated canary analysis (metrics-driven promotion, bake time) with Argo Rollouts/Flagger/Spinnaker; progressive delivery; database migrations during zero-downtime deploys (expand-contract/backward-compatible schema changes — critical and often missed); rollback vs roll-forward; worked examples and a decision table.
