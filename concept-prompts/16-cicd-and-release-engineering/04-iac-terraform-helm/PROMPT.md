...
**Subtopic:** Infrastructure as Code (Terraform & Helm)

Cover at minimum: declarative IaC and why it matters (reproducibility, review, drift detection); Terraform (providers, state — and the critical state-locking/remote-state concerns, plan/apply, modules, workspaces, drift); immutable infrastructure; Helm for Kubernetes (charts, templating, values, releases, the upgrade/rollback model) and its critiques; Kustomize as an alternative; secrets in IaC; testing IaC; worked examples and common pitfalls (state corruption, blast radius of a bad apply).
