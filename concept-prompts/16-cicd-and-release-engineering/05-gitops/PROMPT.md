...
Cover at minimum: the GitOps model (Git as the single source of truth for desired state, a controller continuously reconciles cluster to Git); pull-based vs push-based deploys and why pull is more secure; Argo CD and Flux (how they work, sync, health, drift detection, auto-heal); progressive delivery with GitOps; secrets in GitOps (sealed secrets, SOPS, external secrets); multi-cluster/multi-env repo structure; rollback = git revert; benefits and limitations; worked example.
