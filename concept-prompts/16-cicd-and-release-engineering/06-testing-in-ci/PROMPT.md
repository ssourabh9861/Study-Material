...
**Subtopic:** Testing in CI

Cover at minimum: the test pyramid (unit → integration → e2e) and the inverted ice-cream-cone anti-pattern; what belongs at each level; contract testing (Pact, consumer-driven) for microservices; integration testing with Testcontainers (real DB/Kafka/Redis in CI); test data management; flaky tests (causes, detection, quarantine, fixing) — a major real-world pain; parallelization and test sharding; mutation testing; coverage as a signal not a target; gating deploys on tests; worked examples.
