...
Cover at minimum: the software supply chain threat model (dependency compromise, build tampering — e.g., SolarWinds, xz); dependency management and scanning (SCA, Dependabot, Snyk, transitive deps); SBOM (CycloneDX/SPDX); artifact signing and provenance (Sigstore/cosign, SLSA framework levels, in-toto attestations); securing the build system (least privilege, ephemeral runners, pinned actions); base-image hygiene and vulnerability scanning (Trivy/Grype); secrets scanning; worked examples and a hardening checklist.
