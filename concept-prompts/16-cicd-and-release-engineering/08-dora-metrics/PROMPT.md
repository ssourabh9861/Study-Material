...
**Subtopic:** DORA Metrics

Cover at minimum: the four DORA metrics (deployment frequency, lead time for changes, change failure rate, MTTR/failed-deployment recovery time) — precise definitions and how to measure each; the performance clusters (elite/high/medium/low); what each metric reveals and the failure mode of optimizing one in isolation (gaming); the stability vs throughput false dichotomy (DORA shows they correlate); adding reliability (the 5th metric); using DORA to drive improvement, not punish; worked measurement approach.
