...
**Subtopic:** Tokens & Context Windows

Cover at minimum: what tokens are (subword units, BPE tokenization) and how text↔tokens (≈4 chars/token English, varies by language/code); the context window (input + output share it) and why it's a hard constraint; how pricing works per input/output token; context management strategies (truncation, summarization, sliding window, retrieval); the "lost in the middle" effect and prompt positioning; prompt caching to cut cost/latency; counting tokens (tokenizers); long-context tradeoffs (cost, latency, attention dilution); worked examples.
