...
Cover at minimum: prompt structure (system vs user vs assistant roles); core techniques (zero-shot, few-shot/in-context learning, chain-of-thought, self-consistency, role/persona, output-format constraints, delimiters); structured output (JSON mode, schemas) and reliability; decomposition and prompt chaining; reducing hallucination via grounding/citations; common failure patterns and debugging prompts; prompt templates and versioning as code; evaluation of prompts; worked examples for several tasks.
