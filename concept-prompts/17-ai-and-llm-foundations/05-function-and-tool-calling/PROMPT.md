...
Cover at minimum: what tool/function calling is (the model emits a structured request to call your code; you execute and return results); the request/response loop step by step; defining tools via JSON Schema (good descriptions matter); parallel tool calls; forced vs auto tool choice; structured outputs vs tool calls; handling errors and invalid arguments; security (never trust args, sandbox side effects, permissioning); this is the foundation for agents and MCP (forward reference); worked example of a tool-calling loop in code.
