...
**Subtopic:** LLM Inference & Serving

Cover at minimum: how inference works (prefill vs decode phases, autoregressive generation, the KV cache and why it dominates memory); throughput vs latency metrics (TTFT, inter-token latency, tokens/sec); continuous/in-flight batching (vLLM, the PagedAttention idea); quantization (INT8/INT4, GPTQ/AWQ) for cheaper serving; speculative decoding; serving stacks (vLLM, TGI, TensorRT-LLM); GPU memory math; self-hosted vs API tradeoffs; scaling and autoscaling LLM workloads; worked capacity reasoning.
