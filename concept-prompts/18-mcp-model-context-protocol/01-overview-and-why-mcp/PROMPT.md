...
**Subtopic:** Overview & Why MCP Exists

Cover at minimum: the problem MCP solves — the M×N integration explosion (every model/app × every tool/data source) collapsed into M+N via a standard protocol; the analogy to "USB-C/LSP for AI tools"; what MCP is (an open protocol, introduced by Anthropic late 2024, JSON-RPC 2.0 based) and what it standardizes; how it relates to plain function/tool calling (MCP packages tools behind a reusable server); who adopts it; when MCP is worth it vs a bespoke integration; the ecosystem and current state (flag that it's evolving fast).
