...
Cover at minimum: the transport layer abstraction (JSON-RPC over a transport); stdio transport (local subprocess, how the host launches the server, when to use it); the HTTP-based transport(s) — the older HTTP+SSE and the newer Streamable HTTP (explain the evolution and why it changed); remote vs local servers; session management, reconnection, and streaming of partial results; security per transport (local trust vs remote auth); choosing a transport; worked config for launching a stdio server.
