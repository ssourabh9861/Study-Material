...
**Subtopic:** Prompts Primitive

Cover at minimum: prompts as user-controlled, reusable, parameterized prompt templates exposed by a server (e.g. slash-commands); prompts/list and prompts/get; arguments and how the server returns a sequence of messages; embedding resources into prompts; the control distinction (prompts = user-initiated, tools = model-initiated, resources = app-initiated); use cases (standardized workflows, guided interactions); worked example of a parameterized prompt.
