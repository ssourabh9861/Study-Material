...
**Subtopic:** Auth & Security

Cover at minimum: the MCP security model and trust boundaries; authorization for remote servers (the OAuth2-based authorization spec for HTTP transport — explain the flow); local stdio trust assumptions; the key threats — prompt injection via tool results, malicious/compromised servers, confused-deputy, token theft, over-broad permissions, "tool poisoning"; mitigations (user consent, least privilege, input validation, sandboxing, allow-lists, auditing); supply-chain risk of third-party servers; a security checklist for deploying MCP servers.
