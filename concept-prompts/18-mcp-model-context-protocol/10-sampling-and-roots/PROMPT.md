...
**Subtopic:** Sampling & Roots

Cover at minimum: sampling (server→client requests for an LLM completion — letting a server leverage the host's model, with human-in-the-loop approval, use cases, and why the inversion of control is powerful but sensitive); roots (client-declared filesystem/URI boundaries that scope what a server may access); elicitation (newer: server requesting structured input from the user) if relevant; how these advanced capabilities are negotiated; security implications; worked examples; note these are evolving parts of the spec.
