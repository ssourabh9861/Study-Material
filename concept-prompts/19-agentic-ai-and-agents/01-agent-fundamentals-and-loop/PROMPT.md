...
**Subtopic:** Agent Fundamentals & The Agent Loop

Cover at minimum: what an "agent" actually is (an LLM in a loop with tools, memory, and the autonomy to decide next steps) vs a fixed workflow/chain; the core agent loop (perceive/observe → think/plan → act via tools → observe result → repeat until done); agency as a spectrum (Anthropic's "workflows vs agents" distinction — explain it); when you need an agent vs a deterministic pipeline (and the bias toward the simplest thing that works); components (model, tools, memory, orchestration, stopping criteria); cost/latency/reliability tradeoffs; a minimal agent in code.
