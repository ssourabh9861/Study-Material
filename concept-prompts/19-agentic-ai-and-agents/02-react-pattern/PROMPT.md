...
**Subtopic:** The ReAct Pattern

Cover at minimum: ReAct (Reasoning + Acting) explained — interleaving thought, action, observation; why interleaving reasoning with tool use beats pure chain-of-thought for grounded tasks; the prompt structure (Thought/Action/Observation loop); strengths and failure modes (verbose, can loop, error compounding); ReAct vs plan-and-execute vs function-calling-native agents; parsing/structured variants; a complete worked ReAct trace and code; when to use it today vs newer approaches.
