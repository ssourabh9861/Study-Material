...
Cover at minimum: why agents need memory; short-term/working memory (conversation context, scratchpad) vs long-term memory (vector store, summaries, knowledge); context-window management for long-running agents (summarization, compaction, retrieval of relevant history); episodic vs semantic vs procedural memory; state persistence and resuming interrupted agents (checkpointing — e.g. LangGraph); memory write/read policies and staleness; the cost of carrying state; worked example of an agent with persistent memory.
