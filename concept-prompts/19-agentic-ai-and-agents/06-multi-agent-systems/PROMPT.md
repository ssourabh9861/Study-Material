...
Cover at minimum: when multiple agents beat one (separation of concerns, parallelism, specialized roles) and when they're overkill/add fragility; topologies — orchestrator-worker (supervisor), hierarchical, sequential/pipeline, debate/critique, blackboard; agent-to-agent communication and handoffs; shared vs isolated context and the context-passing cost; coordination and termination; emerging protocols (A2A) at a high level; frameworks (CrewAI, AutoGen, LangGraph multi-agent); failure modes (cascading errors, infinite handoffs); worked example.
