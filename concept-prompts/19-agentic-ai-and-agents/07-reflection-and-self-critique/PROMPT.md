...
**Subtopic:** Reflection & Self-Critique

Cover at minimum: the reflection pattern (the agent reviews/critiques its own output and revises); variants — basic self-refine, Reflexion (learning from feedback across attempts), critic/actor, LLM-as-judge in the loop; tool-grounded reflection (use test results/compiler/validator as the critic — far more reliable than self-judgment); when reflection improves quality vs wastes tokens/loops; stopping criteria; combining with planning; worked example (e.g., code-writing agent that runs tests and fixes failures).
