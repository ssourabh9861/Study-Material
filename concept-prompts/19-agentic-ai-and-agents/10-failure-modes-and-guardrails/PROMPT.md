...
**Subtopic:** Failure Modes & Guardrails

Cover at minimum: the agent-specific failure modes — hallucinated/wrong tool calls, runaway loops (and step/budget limits), error compounding over steps, context overflow, goal drift, getting stuck; security — prompt injection (direct and indirect via tool/web/document content), data exfiltration, the "lethal trifecta" (private data + untrusted content + exfiltration channel), excessive agency/permissions; mitigations — sandboxing, allow-lists, human-in-the-loop approval for destructive actions, output validation, least privilege, spend/step caps, isolation; a production safety checklist; worked guardrail examples.
