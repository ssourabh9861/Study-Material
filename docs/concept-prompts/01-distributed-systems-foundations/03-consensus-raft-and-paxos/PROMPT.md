...
**Subtopic:** Consensus — Raft & Paxos

Cover at minimum: what consensus is and why it's hard (FLP impossibility — explain it); Raft in full (leader election, terms, log replication, commit index, safety, membership changes, snapshotting); Paxos/Multi-Paxos at a working level and why Raft was created; where consensus is actually used (etcd, ZooKeeper/ZAB, Kafka KRaft, Spanner); when you do NOT need consensus (quorums, CRDTs); performance cost of consensus.
