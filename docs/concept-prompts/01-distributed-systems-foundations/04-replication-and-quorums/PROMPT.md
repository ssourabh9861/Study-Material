...
Cover at minimum: single-leader, multi-leader, leaderless replication; sync vs async vs semi-sync; quorum math (R + W > N), sloppy quorums, hinted handoff; read-repair and anti-entropy (Merkle trees); replication lag and the anomalies it creates; conflict resolution (LWW, vector clocks, CRDTs); how Dynamo, Cassandra, and Postgres replication work.
