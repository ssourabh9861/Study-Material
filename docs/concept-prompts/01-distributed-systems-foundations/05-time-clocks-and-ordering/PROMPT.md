...
**Subtopic:** Time, Clocks & Ordering

Cover at minimum: why wall-clock time is unreliable in distributed systems (NTP skew, leap seconds); logical clocks — Lamport timestamps and vector clocks (with worked examples); hybrid logical clocks (HLC); Google TrueTime and how Spanner uses commit-wait; happens-before relation; how ordering underpins causal consistency and conflict resolution.
