...
Cover at minimum: sagas as a sequence of local transactions + compensations; orchestration vs choreography (with diagrams in text and tradeoffs); compensating transactions and semantic rollback; isolation anomalies in sagas (dirty reads, lost updates) and countermeasures (semantic locks, commutative updates, reread); implementing sagas with a state machine / orchestrator (e.g., Temporal, Camunda, custom); failure and retry handling; worked example for an order/payment flow.
