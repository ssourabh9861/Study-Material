...
**Subtopic:** Delivery Semantics

Cover at minimum: at-most-once, at-least-once, exactly-once — precise definitions and what each requires; why true exactly-once delivery is impossible but effectively-once processing is achievable; acks, retries, and duplicates; how Kafka/SQS/RabbitMQ implement each; the role of idempotency and transactions; end-to-end semantics across a pipeline; choosing the right semantic for a use case.
