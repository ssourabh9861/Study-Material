...
Cover at minimum: when you need a distributed lock vs when it's an anti-pattern; implementations — Redis SETNX/Redlock (and Martin Kleppmann's critique), ZooKeeper ephemeral nodes, etcd leases, DB row locks; the fencing-token problem (why locks alone are unsafe with GC pauses/clock skew); lease expiry and liveness vs safety; leader election as a lock; worked examples and failure scenarios.
