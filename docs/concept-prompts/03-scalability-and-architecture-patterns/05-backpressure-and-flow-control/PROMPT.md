...
Cover at minimum: what backpressure is and why unbounded buffering causes metastable failures/OOM; push vs pull; bounded queues and queue-based load leveling; Reactive Streams / Project Reactor demand signaling; TCP flow control as an analogy; backpressure across service boundaries (rejecting vs buffering vs shedding); credit-based flow control; worked examples in Java (BlockingQueue, Reactor).
