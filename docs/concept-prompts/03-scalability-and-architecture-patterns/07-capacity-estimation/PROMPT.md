...
Cover at minimum: back-of-the-envelope method; key numbers to memorize (latency numbers every programmer should know, QPS per box, storage per row, bandwidth); estimating QPS, storage, bandwidth, memory, and number of servers; read/write ratios and fan-out; peak vs average and headroom; worked examples (design a URL shortener/newsfeed capacity); how to present estimates in an interview.
