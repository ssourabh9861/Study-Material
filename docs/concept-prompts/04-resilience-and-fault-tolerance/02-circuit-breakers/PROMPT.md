...
Cover at minimum: the circuit-breaker pattern (closed/open/half-open state machine, thresholds, sliding windows); how it prevents cascading failure; Hystrix (now in maintenance) vs Resilience4j vs Sentinel; configuration (failure-rate threshold, slow-call detection, wait duration, permitted calls in half-open); fallbacks; combining with bulkhead/timeout/retry; metrics to monitor; worked Resilience4j Java example; common misconfigurations.
