...
Cover at minimum: principles of chaos engineering (steady-state hypothesis, blast radius, run in prod carefully); the history (Netflix Chaos Monkey/Simian Army); failure injection types (latency, errors, instance kill, network partition, resource exhaustion); tools (Chaos Monkey, Gremlin, LitmusChaos, AWS FIS); game days; how to start safely and measure; what good observability must exist first; organizational adoption.
