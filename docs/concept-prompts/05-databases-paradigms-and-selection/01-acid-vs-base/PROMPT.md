...
Cover at minimum: ACID defined precisely (atomicity, consistency, isolation, durability — what each really guarantees and what databases do to provide it: WAL, fsync, locks/MVCC); BASE (basically available, soft state, eventual consistency); the spectrum between them; when eventual consistency is acceptable vs unacceptable (money vs feeds); how distributed databases blur the line (NewSQL — Spanner, CockroachDB); durability nuances (fsync, group commit, replication).
