...
**Subtopic:** Document & Graph Databases

Cover at minimum: document model (MongoDB) — schema design, embedding vs referencing, indexes, aggregation pipeline, transactions, sharding; when documents fit and when they don't; graph databases (Neo4j, property graph, Cypher) — when relationships are first-class (fraud, social, recommendations); graph vs relational joins at depth; traversal performance; choosing document vs graph vs relational with examples.
