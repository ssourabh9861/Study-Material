...
**Concept area:** Database Paradigms & Choosing the Right Store
**Subtopic:** Isolation Levels & MVCC

Cover at minimum: the four SQL isolation levels and the anomalies each prevents (dirty/non-repeatable/phantom reads) plus write-skew and lost updates; how isolation is implemented (2-phase locking vs MVCC); MVCC in depth (version chains, snapshots, visibility, vacuum/GC of old versions); snapshot isolation vs serializable (SSI); Postgres vs MySQL InnoDB behavior; choosing an isolation level; SELECT ... FOR UPDATE and optimistic vs pessimistic locking.
