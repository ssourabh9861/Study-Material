...
Cover at minimum: a repeatable decision framework driven by access patterns (read/write ratio, query shapes, consistency needs, latency budget, scale, durability, cost); polyglot persistence; mapping common workloads to stores (ledger, feed, session, search, analytics, time-series, graph); the cost of choosing wrong and migration pain; a decision table/flowchart; worked examples justifying a choice the way an interviewer wants.
