...
Cover at minimum: the problem with mod-N hashing (mass remap on resize); consistent hashing derived from scratch (the ring, mapping keys and nodes); virtual nodes and why they're needed (balance, heterogeneous capacity); adding/removing nodes and the fraction of keys moved; bounded-load consistent hashing; rendezvous (HRW) hashing as an alternative; where it's used (Dynamo, Cassandra, memcached clients, load balancers); a worked Java implementation.
