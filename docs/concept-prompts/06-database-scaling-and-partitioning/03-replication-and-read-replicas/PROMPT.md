...
Cover at minimum: leader-follower replication mechanics (statement vs row-based vs WAL shipping); sync/async/semi-sync and the durability tradeoff; read replicas for read scaling and their limits; replica promotion/failover (and tools like Orchestrator, Patroni); routing reads vs writes; cascading replicas; multi-leader and its conflict problems; backups vs replicas; worked setup and gotchas.
