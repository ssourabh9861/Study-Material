...
Cover at minimum: what causes replication lag; the anomalies it creates (read-your-writes violation, monotonic-read violation, stale reads); mitigations — read-from-leader-after-write, sticky routing, version/timestamp tokens, monotonic read tokens, bounded staleness; measuring lag; the "I just posted but can't see it" class of bugs; when stale reads are fine and when they corrupt logic; concrete patterns and code.
