...
Cover at minimum: why resharding is hard and how to do it with zero downtime; the dual-write + backfill + shadow-read + cutover playbook; double-reading and reconciliation; changing a shard key; expand-in-place vs migrate-to-new-cluster; using CDC to migrate databases (e.g., MySQL→new store) with verification; rollback strategy; real migration war stories; checklist for a safe migration.
