...
Cover at minimum: cache-aside (lazy), read-through, write-through, write-behind/write-back, refresh-ahead — each explained with sequence of operations, consistency implication, and failure behavior; read vs write path; where the cache lives (client, local, distributed, CDN); TTL design; cache warming; choosing a pattern per workload; worked Java examples (Caffeine + Redis) for each.
