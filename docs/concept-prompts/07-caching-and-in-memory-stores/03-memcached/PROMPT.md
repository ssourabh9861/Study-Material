...
Cover at minimum: Memcached architecture (multi-threaded, slab allocator, LRU); how it differs from Redis (no persistence, no rich types, no replication, simpler/faster for pure cache); slab allocation and memory fragmentation/calcification; client-side consistent hashing across nodes; when Memcached is the better choice; multiget; eviction; sizing; real usage (Facebook's scaled memcache, leases, regional pools).
