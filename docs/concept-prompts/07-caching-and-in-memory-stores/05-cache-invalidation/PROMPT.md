...
Cover at minimum: why invalidation is "one of the two hard problems"; strategies — TTL expiry, write-through invalidation, explicit delete on write, versioned keys, event/CDC-driven invalidation, generational keys; the staleness budget concept; invalidation across multiple cache layers and regions; the race between DB write and cache update (and how to avoid serving stale forever); tagging/grouped invalidation; worked examples.
