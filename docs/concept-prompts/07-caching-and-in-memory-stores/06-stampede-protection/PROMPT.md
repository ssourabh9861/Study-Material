...
**Subtopic:** Stampede Protection

Cover at minimum: cache stampede/dogpile defined; mitigations in depth — per-key mutex/lock, single-flight/request coalescing, probabilistic early recomputation (XFetch), staggered/jittered TTLs, serve-stale-while-revalidate, background refresh; cold-cache and mass-expiry herds; combining with negative caching; worked Java + Redis implementation; tradeoffs of each approach.
