...
Cover at minimum: layered caching (CPU→app/local→distributed→CDN→browser) and coherence across layers; cache↔DB consistency strategies and their failure windows; the read-after-write problem with caches; negative caching; multi-layer invalidation and propagation delay; CDN caching (cache keys, surrogate keys, purging, stale-while-revalidate/stale-if-error); consistency vs latency tradeoff; concrete end-to-end design.
