...
**Subtopic:** Consumers & Consumer Groups

Cover at minimum: the poll loop and consumer internals; consumer groups and partition assignment (max parallelism = partitions); offset management (__consumer_offsets, auto vs manual commit, commitSync vs commitAsync, at-least-once default); consumer lag (definition, monitoring); assignment strategies (range, round-robin, sticky, cooperative-sticky); static membership; key configs (max.poll.records, max.poll.interval.ms, session.timeout.ms, heartbeat); full Java consumer examples.
