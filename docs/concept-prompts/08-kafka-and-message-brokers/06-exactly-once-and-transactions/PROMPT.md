...
**Subtopic:** Exactly-Once & Transactions

Cover at minimum: why "exactly once" is subtle; the three layers — idempotent producer, transactions (transactional.id, producer epochs, the transaction coordinator, control records), and consumer read_committed/LSO; the consume-process-produce loop with sendOffsetsToTransaction; what EOS does and does NOT cover (the external-DB boundary); performance cost; when to prefer at-least-once + idempotency keys instead; full transactional Java example.
