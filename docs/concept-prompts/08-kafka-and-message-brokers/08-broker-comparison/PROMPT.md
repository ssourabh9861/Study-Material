...
**Subtopic:** Broker Comparison (Kafka vs RabbitMQ vs SQS/SNS vs Pulsar)

Cover at minimum: the fundamental model difference (log vs queue vs managed); per-broker deep notes — RabbitMQ (exchanges, bindings, routing, competing consumers, AMQP), SQS/SNS (managed, standard vs FIFO, visibility timeout, DLQ), Pulsar (segments on BookKeeper, subscription types exclusive/shared/failover/key_shared, tiered storage, multi-tenancy, geo-replication); a decision table across ordering, throughput, replay, delivery, ops; how to choose; explain each unfamiliar term.
