...
**Concept area:** Kafka & Message Brokers
**Subtopic:** Operations & Troubleshooting

Cover at minimum: the operational metrics that matter (consumer lag, under-replicated partitions, ISR shrink/expand, request latency, log flush); diagnosing and fixing rebalance storms, hot partitions, poison messages (retry topics + DLQ), under-replicated partitions, disk pressure; partition count planning and the pain of changing it; topic configuration; schema registry + compatibility (BACKWARD/FORWARD/FULL); CLI tools (kafka-consumer-groups, kafka-topics); capacity planning; real incident patterns.
