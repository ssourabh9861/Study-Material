...
**Subtopic:** Collections Framework Internals

Cover at minimum: the Collection/Map hierarchy; ArrayList vs LinkedList (and why LinkedList is usually wrong); HashMap internals (buckets, hashing, treeification at 8, resize/rehash, load factor 0.75); LinkedHashMap (access-order for LRU); TreeMap (red-black tree); ArrayDeque; identity vs equals/hashCode contract; fail-fast iterators and ConcurrentModificationException; choosing the right collection; Big-O cheat table; worked examples.
