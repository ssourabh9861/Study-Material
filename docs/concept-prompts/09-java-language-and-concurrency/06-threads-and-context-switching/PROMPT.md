...
**Subtopic:** Threads & Context Switching

Cover at minimum: what a thread is (OS thread vs JVM thread, 1:1 mapping for platform threads); thread lifecycle/states (NEW, RUNNABLE, BLOCKED, WAITING, TIMED_WAITING, TERMINATED); how the OS scheduler works (time slices, preemption, priorities, run queues) — explain from scratch; **context switching** in depth: what it is, what gets saved/restored, voluntary vs involuntary, the cost (direct: register/stack save; indirect: cache/TLB pollution — typically ~1–10µs), and how to measure it (vmstat, pidstat, perf); thread creation cost and why pools exist; CPU-bound vs IO-bound and how each interacts with the scheduler; oversubscription and thrashing; mapping to virtual threads (forward reference).
