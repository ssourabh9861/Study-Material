...
**Subtopic:** Synchronization & Locks

Cover at minimum: the mutual-exclusion problem; intrinsic locks (synchronized, monitor, wait/notify, the object monitor and entry/wait sets); ReentrantLock (fairness, tryLock, lockInterruptibly, conditions); ReadWriteLock and StampedLock (optimistic reads); lock granularity, coarse vs fine; reentrancy; lock ordering to avoid deadlock; biased/lightweight/heavyweight lock evolution and lock elision; Condition vs wait/notify; worked examples (bounded buffer, etc.).
