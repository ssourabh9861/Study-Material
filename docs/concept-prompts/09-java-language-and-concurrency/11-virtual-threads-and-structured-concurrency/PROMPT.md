...
**Subtopic:** Virtual Threads & Structured Concurrency

Cover at minimum: Project Loom motivation (thread-per-request without the thread cost); virtual threads vs platform threads (carrier threads, mounting/unmounting, the scheduler); what makes them cheap and where they help (IO-bound, high concurrency) vs where they don't (CPU-bound); **pinning** (synchronized, native calls) and how to avoid it; structured concurrency (StructuredTaskScope) for reliable fan-out; ScopedValue vs ThreadLocal; migration guidance; production caveats; worked examples (Java 21).
