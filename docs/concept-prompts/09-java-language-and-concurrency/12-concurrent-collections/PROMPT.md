...
Cover at minimum: ConcurrentHashMap internals (bucket-level concurrency, no segment locks since Java 8, computeIfAbsent atomicity, size() approximation); CopyOnWriteArrayList/Set (read-heavy, write cost); BlockingQueue family (ArrayBlockingQueue, LinkedBlockingQueue, SynchronousQueue, PriorityBlockingQueue, DelayQueue) and producer-consumer patterns; ConcurrentLinkedQueue (lock-free Michael-Scott); ConcurrentSkipListMap; when to use which; pitfalls (compound actions aren't atomic); worked examples.
