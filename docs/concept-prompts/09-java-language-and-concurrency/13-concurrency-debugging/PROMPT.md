...
**Subtopic:** Concurrency Debugging

Cover at minimum: the bug taxonomy (deadlock, livelock, starvation, race condition, visibility bug, atomicity violation); diagnosing deadlocks from thread dumps (jstack, finding the cycle); detecting races (code review heuristics, jcstress, TSAN-style tools); reproducing heisenbugs; visibility bugs from missing volatile; using JFR/async-profiler for lock contention and thread states; classic interview deadlock puzzles; a checklist for reviewing concurrent code.
