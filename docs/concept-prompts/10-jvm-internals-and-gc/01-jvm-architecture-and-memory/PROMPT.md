...
**Subtopic:** JVM Architecture & Memory Areas

Cover at minimum: the JVM runtime data areas (heap, young/old gen, Metaspace, thread stacks, PC register, native/direct memory); object header layout (mark word, klass pointer), compressed oops; how bytecode is loaded→verified→executed; the execution engine (interpreter + JIT); stack frames; where different things are allocated; -Xmx/-Xms/-Xss and key memory flags with defaults; how to inspect (jcmd, jmap, NMT).
