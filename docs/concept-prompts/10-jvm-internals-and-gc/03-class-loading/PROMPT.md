...
**Subtopic:** Class Loading & ClassLoaders

Cover at minimum: the loading→linking(verify/prepare/resolve)→initialization lifecycle; the classloader hierarchy (bootstrap, platform/ext, application) and the parent-delegation model; custom classloaders and use cases (plugins, app servers, hot reload); class identity = (name, classloader); common failures (ClassNotFoundException vs NoClassDefFoundError, LinkageError, ClassCastException across loaders, jar hell); Metaspace and class unloading; worked examples.
