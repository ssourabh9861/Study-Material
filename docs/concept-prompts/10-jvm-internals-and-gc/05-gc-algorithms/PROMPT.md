...
**Subtopic:** GC Algorithms (G1, ZGC, Shenandoah, Parallel)

Cover at minimum: Serial/Parallel (throughput) collectors; G1 in depth (regions, concurrent marking, mixed collections, pause-time goal, humongous objects); ZGC (colored pointers, load barriers, concurrent relocation, sub-millisecond pauses, generational ZGC); Shenandoah (Brooks/load-reference barriers, concurrent compaction); a comparison table (pause, throughput, footprint, heap size sweet spot, JDK availability); how to choose a collector for a workload; defaults per JDK version.
