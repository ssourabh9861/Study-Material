...
**Subtopic:** GC Tuning & Reading GC Logs

Cover at minimum: enabling and reading unified GC logs (-Xlog:gc*); the key signals (pause times, frequency, allocation rate, promotion rate, heap occupancy after GC, humongous allocations); sizing heap and generations; pause-time goals (MaxGCPauseMillis); when tuning is needed vs when to just add memory/change collector; common anti-patterns (over-tuning, premature flags); tools (GCeasy, JClarity-style analysis); a tuning methodology.
