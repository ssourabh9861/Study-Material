...
**Subtopic:** OOM & Memory Leak Diagnosis

Cover at minimum: the OutOfMemoryError variants (Java heap space, GC overhead limit, Metaspace, Direct buffer memory, unable to create native thread) and what each means; capturing heap dumps (-XX:+HeapDumpOnOutOfMemoryError, jmap, jcmd); analyzing with Eclipse MAT (dominator tree, leak suspects, retained heap); classic leak patterns (static collections, ThreadLocal, unclosed resources, classloader leaks, listener leaks); off-heap/direct memory leaks; a step-by-step investigation playbook.
