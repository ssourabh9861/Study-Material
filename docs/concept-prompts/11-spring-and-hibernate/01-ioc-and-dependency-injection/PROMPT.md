...
Cover at minimum: inversion of control and dependency injection explained from first principles (and why they reduce coupling); the Spring container (BeanFactory vs ApplicationContext); bean definition via annotations/Java config/XML; injection types (constructor — preferred, setter, field) and why constructor injection wins; @Component/@Service/@Repository/@Configuration/@Bean; component scanning; qualifiers/@Primary; the ApplicationContext lifecycle; worked examples and common mistakes (field injection, circular deps).
