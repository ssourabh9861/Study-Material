...
**Subtopic:** Bean Lifecycle & Scopes

Cover at minimum: the full bean lifecycle (instantiation → populate → aware callbacks → BeanPostProcessor before → @PostConstruct/InitializingBean → after → ready → @PreDestroy); BeanPostProcessor and BeanFactoryPostProcessor (and how AOP/auto-config hook in here); bean scopes (singleton, prototype, request, session, application, websocket) and the singleton-injecting-prototype problem (ObjectProvider/proxy); lazy initialization; circular dependency resolution; worked examples.
