...
**Subtopic:** Aspect-Oriented Programming (AOP)

Cover at minimum: the cross-cutting-concern problem AOP solves (logging, tx, security, metrics); core terms (aspect, join point, pointcut, advice types before/after/around, weaving); Spring AOP proxy-based model (JDK dynamic proxies vs CGLIB) and its big limitations (self-invocation bypasses the proxy, only public methods); AspectJ vs Spring AOP; how @Transactional/@Async/@Cacheable are implemented via AOP; pointcut expressions; worked custom-aspect example; gotchas.
