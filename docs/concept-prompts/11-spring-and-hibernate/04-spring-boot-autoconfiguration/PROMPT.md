...
Cover at minimum: what Spring Boot adds over Spring; auto-configuration mechanism (@EnableAutoConfiguration, spring.factories/AutoConfiguration.imports, @Conditional* classes) explained step by step; starters and dependency management; externalized configuration (properties/YAML, profiles, @ConfigurationProperties, property source order); the embedded server; Actuator basics; how to debug/override auto-config (conditions report, excludes); building a custom starter; worked examples.
