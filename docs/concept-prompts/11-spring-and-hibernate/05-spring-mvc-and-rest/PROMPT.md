...
**Subtopic:** Spring MVC & REST

Cover at minimum: the DispatcherServlet request lifecycle in full (handler mapping → handler adapter → controller → view/message converter → response); @RestController/@RequestMapping/@GetMapping etc.; argument resolvers and HttpMessageConverters (Jackson); validation (@Valid, Bean Validation); exception handling (@ExceptionHandler, @ControllerAdvice, ProblemDetail); content negotiation; filters vs interceptors vs AOP; building a clean REST API; worked examples.
