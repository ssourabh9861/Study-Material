...
**Subtopic:** Spring WebFlux & Reactive

Cover at minimum: blocking vs non-blocking/reactive model and when each wins; Reactive Streams spec (Publisher/Subscriber/Subscription/Processor, backpressure); Project Reactor (Mono/Flux, operators, schedulers, hot vs cold); WebFlux vs MVC (Netty event loop, threading); the cost of going reactive (debuggability, ecosystem, blocking-call hazards); when reactive is NOT worth it (especially with virtual threads now available); worked examples and pitfalls (blocking inside reactive).
