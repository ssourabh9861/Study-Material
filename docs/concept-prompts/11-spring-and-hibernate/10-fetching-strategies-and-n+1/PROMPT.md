...
**Subtopic:** Fetching Strategies & the N+1 Problem

Cover at minimum: lazy vs eager fetching and proxies (how lazy loading works, LazyInitializationException outside a session); the N+1 select problem explained with SQL traces; fixes — JOIN FETCH, @EntityGraph, batch fetching (@BatchSize, hibernate.default_batch_fetch_size), subselect, DTO projections; FetchType defaults per relationship; pagination + fetch join pitfalls (in-memory paging warning); the open-session-in-view anti-pattern; how to detect N+1 (SQL logging, Hypersistence/datasource-proxy); worked examples.
