...
**Concept area:** Spring & Hibernate
**Subtopic:** Transactions & Spring @Transactional

Cover at minimum: declarative transactions via @Transactional (and the AOP proxy mechanism behind it); propagation levels (REQUIRED, REQUIRES_NEW, NESTED, SUPPORTS, MANDATORY, NEVER, NOT_SUPPORTED) with concrete scenarios; isolation levels mapping to the DB; rollback rules (only unchecked by default — a classic bug); the self-invocation pitfall (calling @Transactional method from same class bypasses it); read-only optimization; transaction synchronization; second-level/query cache interaction; connecting to JPA flush/commit; worked examples and common bugs.
