...
**Subtopic:** Spring Security & Testing

Cover at minimum: Spring Security architecture (the filter chain, SecurityContext, Authentication/Authorities, AuthenticationManager/Provider, UserDetailsService); form login vs stateless JWT/OAuth2 resource server; method security (@PreAuthorize); CSRF, CORS, password encoding; common misconfigurations; testing — the pyramid in Spring (@WebMvcTest slice, @DataJpaTest, @SpringBootTest, MockMvc, Testcontainers for real DB/Kafka, @MockBean); testing secured endpoints; worked examples for both security and tests.
