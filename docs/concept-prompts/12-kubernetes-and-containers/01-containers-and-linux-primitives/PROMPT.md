...
**Subtopic:** Containers & Linux Primitives

Cover at minimum: what a container actually is (a process, not a VM); the Linux primitives explained from scratch — namespaces (pid, net, mnt, uts, ipc, user) for isolation, cgroups (v1 vs v2) for resource limits, union/overlay filesystems for layered images; OCI image format and layers; container runtimes (Docker, containerd, runc, CRI); building good images (multi-stage, small base, layer caching, non-root); containers vs VMs tradeoffs; cgroup-aware JVM behavior.
