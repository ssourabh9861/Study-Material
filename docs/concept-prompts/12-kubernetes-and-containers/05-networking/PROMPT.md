...
Cover at minimum: the Kubernetes network model (every pod gets a routable IP, no NAT between pods); Services (ClusterIP, NodePort, LoadBalancer, ExternalName, headless) and how kube-proxy implements them (iptables vs IPVS); Endpoints/EndpointSlices; cluster DNS (CoreDNS) and service discovery; Ingress and ingress controllers; the CNI plugin model (Calico, Cilium/eBPF); NetworkPolicies; service mesh interaction; debugging connectivity; worked examples.
