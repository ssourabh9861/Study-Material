...
Cover at minimum: ephemeral vs persistent storage; Volumes, PersistentVolume/PersistentVolumeClaim, StorageClass and dynamic provisioning; access modes (RWO/ROX/RWX) and what they really allow; the CSI driver model; StatefulSet volumeClaimTemplates and stable per-pod storage; reclaim policies; stateful workloads on K8s (should you?); local vs networked storage tradeoffs; backup/snapshots; worked examples.
