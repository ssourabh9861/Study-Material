...
Cover at minimum: Horizontal Pod Autoscaler (metrics, the scaling algorithm/formula, custom & external metrics, stabilization windows); Vertical Pod Autoscaler (and why HPA+VPA on the same metric conflict); Cluster Autoscaler (node scale-up/down, scale-from-zero); KEDA for event-driven scaling (e.g., Kafka lag); the interaction between the three; cold-start and scale-up latency; setting requests so autoscaling works; worked HPA example and pitfalls.
