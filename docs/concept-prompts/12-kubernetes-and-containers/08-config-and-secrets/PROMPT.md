...
**Subtopic:** Config & Secrets

Cover at minimum: ConfigMaps and Secrets (and that Secrets are only base64, not encrypted by default — enable encryption-at-rest); injecting config via env vars vs mounted files (and the live-reload difference); immutable ConfigMaps; external secret managers (Vault, External Secrets Operator, cloud CSI secret drivers); rotating secrets without downtime; the 12-factor config principle; RBAC around secrets; worked examples.
