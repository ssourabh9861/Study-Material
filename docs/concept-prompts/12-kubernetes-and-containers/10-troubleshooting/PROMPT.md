...
Cover at minimum: a systematic debugging method (kubectl describe/logs/events/exec, ephemeral debug containers); the common failure states and their root causes — CrashLoopBackOff, OOMKilled (exit 137), ImagePullBackOff/ErrImagePull, Pending (unschedulable: resources/taints/affinity/PVC), Evicted (node pressure), readiness failures, DNS issues; reading events and exit codes; node-level issues (disk/memory pressure, NotReady); a triage flowchart; real scenarios.
