...
**Subtopic:** The Three Pillars

Cover at minimum: monitoring vs observability (known-unknowns vs unknown-unknowns); the three pillars — metrics, logs, traces — what each is good and bad at, cost/cardinality profile, and why you need all three; events/profiles as emerging pillars; high-cardinality observability; correlation across pillars (exemplars, trace IDs in logs); the cost problem and sampling; what to instrument first; a mental model for choosing the right signal for a question.
