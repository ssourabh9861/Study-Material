...
Cover at minimum: the Prometheus model (pull-based scraping, time series identified by name+labels, the TSDB); metric types (counter, gauge, histogram, summary — when to use each and how histograms enable percentiles); cardinality explosions (the #1 way to blow up Prometheus) and how to avoid them; PromQL essentials (rate, histogram_quantile, aggregation, joins); exporters and client instrumentation (Micrometer in Spring); recording & alerting rules; remote-write/long-term storage (Thanos/Cortex/Mimir); pull vs push (Pushgateway); worked examples.
