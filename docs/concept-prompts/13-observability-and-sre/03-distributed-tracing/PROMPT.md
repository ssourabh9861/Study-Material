...
Cover at minimum: why tracing exists (following a request across services); core concepts (trace, span, span context, parent/child, attributes, events) ; context propagation (W3C traceparent header, B3) across HTTP/messaging/threads; OpenTelemetry (the standard: API/SDK/Collector, auto vs manual instrumentation); sampling strategies (head vs tail, probabilistic, rate-limiting) and their tradeoffs; backends (Jaeger, Tempo, Zipkin); correlating traces with logs/metrics; cost; instrumenting a Java service; worked examples.
