...
**Subtopic:** Logging & Aggregation

Cover at minimum: structured logging (JSON) and why it beats string logs; correlation/trace IDs and MDC; log levels and what to log (and what never to log — PII/secrets); centralized aggregation pipelines — ELK/Elastic (Logstash/Beats/Kibana), Loki (label-based, cheaper), Splunk; log volume/cost control (sampling, levels, retention tiers); parsing/indexing tradeoffs; querying logs effectively; the difference between logs and events/metrics; worked logback/JSON config.
