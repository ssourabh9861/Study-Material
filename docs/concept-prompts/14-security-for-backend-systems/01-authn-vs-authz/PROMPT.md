...
Cover at minimum: the precise distinction (who you are vs what you can do); authentication factors and methods (passwords, MFA/TOTP, WebAuthn/passkeys, certificates); session-based vs token-based auth and where each belongs (browser vs service mesh); authorization models (RBAC, ABAC, ReBAC/Zanzibar, ACLs) with tradeoffs; centralized vs decentralized authz (policy engines like OPA); where to enforce in microservices (gateway vs service); worked examples.
