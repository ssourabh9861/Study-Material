...
Cover at minimum: the problem OAuth2 solves (delegated authorization) and the roles (resource owner, client, authorization server, resource server); grant types — authorization code + PKCE (the modern default, explain PKCE), client credentials (service-to-service), device code, and the deprecated implicit/password grants (and why); access vs refresh tokens; scopes/consent; OIDC layered on top (ID token, userinfo, the difference from OAuth2); common flows end to end; security pitfalls (redirect URI validation, token leakage); worked examples.
