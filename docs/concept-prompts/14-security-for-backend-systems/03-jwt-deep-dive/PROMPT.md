...
**Subtopic:** JWT Deep Dive

Cover at minimum: JWT structure (header.payload.signature, base64url, standard claims iss/sub/aud/exp/iat/nbf/jti); signing algorithms (HS256 symmetric vs RS256/ES256 asymmetric) and when to use which; verification step by step; key distribution via JWKS and key rotation (kid); the revocation problem (stateless tokens can't be revoked — short TTL + refresh, denylists, token introspection); critical vulnerabilities (alg:none, alg confusion RS→HS, weak secrets, missing aud/exp checks); JWT vs opaque tokens + introspection; worked validation code.
