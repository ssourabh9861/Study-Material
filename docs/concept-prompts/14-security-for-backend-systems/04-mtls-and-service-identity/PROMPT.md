...
**Subtopic:** mTLS & Service Identity

Cover at minimum: TLS handshake basics (then mutual TLS where the client also presents a cert); why service-to-service mTLS matters in zero-trust networks; certificate issuance and rotation (short-lived certs); SPIFFE/SPIRE and SVIDs for workload identity; how a service mesh (Istio/Linkerd) automates mTLS via sidecars; certificate pinning; PKI basics (CA, chain of trust, CRL/OCSP); performance considerations; worked configuration and the failure modes (expired certs cause outages).
