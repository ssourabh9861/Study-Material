...
**Subtopic:** Request Integrity & Replay Protection

Cover at minimum: ensuring a request is authentic and untampered; HMAC request/webhook signing (how it works, shared secret, what to sign — body+timestamp+path) and verification (constant-time compare); replay attacks and defenses (nonces, timestamps + tolerance window, idempotency keys); webhook security end to end (signature verification, the GitHub/Stripe model); message authentication vs encryption; preventing CSRF; worked Java HMAC verify example; common mistakes (signing the wrong thing, timing attacks).
