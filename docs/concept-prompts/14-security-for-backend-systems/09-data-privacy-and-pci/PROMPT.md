...
**Subtopic:** Data Privacy & PCI Compliance

Cover at minimum: classifying data (PII, PHI, PCI cardholder data); core privacy regulations at a working level (GDPR rights — access/erasure/portability; data minimization; data residency); PCI-DSS basics and the key senior move of **scope reduction** (tokenization, isolating the cardholder-data environment, never storing PAN/CVV, using a PCI-compliant vault/PSP); audit logging and tamper-evidence; encryption + access control + retention policies; pseudonymization vs anonymization; right-to-be-forgotten in event-sourced/immutable systems (crypto-shredding); worked patterns.
