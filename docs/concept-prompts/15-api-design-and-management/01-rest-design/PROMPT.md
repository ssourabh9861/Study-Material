...
**Subtopic:** REST Design

Cover at minimum: REST principles and the Richardson Maturity Model; resource modeling and URI design; correct HTTP method semantics and idempotency/safety; status codes (the ones that matter and common misuses); request/response design, HATEOAS (and why it's rarely used); filtering/sorting/field selection; PATCH (JSON Patch vs merge patch); content negotiation; designing for evolvability; HTTP caching headers (ETag, Cache-Control, conditional requests); a worked, well-designed REST API.
