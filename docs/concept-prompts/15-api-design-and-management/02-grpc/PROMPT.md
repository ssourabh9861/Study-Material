...
Cover at minimum: what gRPC is (HTTP/2 + Protocol Buffers RPC); Protobuf (IDL, wire format, why it's compact, codegen); the four call types (unary, server streaming, client streaming, bidirectional) with use cases; HTTP/2 multiplexing/streaming benefits; deadlines/cancellation, metadata, interceptors, error model (status codes); load balancing gRPC (L7, why L4 fails); gRPC vs REST vs GraphQL decision; gRPC-Web and gateways; worked Java example (proto + service + client).
