...
**Concept area:** API Design & Management
**Subtopic:** Error Handling & Idempotency

Cover at minimum: designing a consistent error model (RFC 7807/9457 Problem Details, error codes, machine vs human messages, correlation IDs); mapping exceptions to status codes; partial failure and how to communicate it; idempotency for write APIs (idempotency keys — header, storage, response replay, scope, TTL) à la Stripe; safe retries and the client/server contract; distinguishing retryable vs non-retryable errors; worked Java examples for an idempotent endpoint and a uniform error handler.
