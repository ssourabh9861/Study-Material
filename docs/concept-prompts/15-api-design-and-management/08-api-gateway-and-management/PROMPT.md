...
**Subtopic:** API Gateway & Management

Cover at minimum: API gateway responsibilities (routing, auth, rate limiting, transformation, aggregation, TLS, caching) and the anti-pattern of putting business logic in it; the full API management lifecycle (design, mock, publish, developer portal, keys, analytics, monetization); popular gateways (Kong, Apigee, AWS API Gateway, Spring Cloud Gateway); BFF vs gateway; gateway as a single point of failure (HA); north-south vs east-west traffic; worked gateway config example.
