...
**Subtopic:** Rate Limiting & Quotas

Cover at minimum: rate limits vs quotas vs spike arrest; per-key/per-user/per-tenant/global dimensions; the algorithms (token bucket, sliding window — cross-reference) implemented at the API layer; distributed enforcement (Redis, gateway-native); response design (429, Retry-After, X-RateLimit-* headers); fairness and tiered plans; protecting against abusive clients and DDoS; graceful degradation when limits hit; worked gateway + Redis example.
