...
Cover at minimum: what an LLM is for an engineer (a next-token predictor over a transformer) — explain transformers/attention at a working, non-mathematical-overload level; pretraining vs instruction-tuning vs RLHF; what models can and cannot do (no real-time knowledge, hallucination, no true reasoning vs reasoning models); decoding/sampling parameters (temperature, top-p, top-k, max tokens, stop sequences); deterministic vs creative outputs; model families and sizes; the API mental model (stateless completions, you manage context); cost/latency basics.
