...
**Subtopic:** Embeddings & Vector Search

Cover at minimum: what embeddings are (dense vectors capturing semantic meaning); embedding models and dimensions; similarity metrics (cosine, dot, Euclidean — when each); vector databases (Pinecone, Weaviate, Milvus, pgvector, Elasticsearch) and what they add over a flat index; approximate nearest neighbor algorithms — HNSW (graph-based) and IVF/PQ (explain each, the recall/latency/memory tradeoff); indexing and filtering (hybrid metadata + vector); chunking impact on embeddings; worked example (embed → store → query).
