...
Cover at minimum: why RAG exists (ground the model in your data, reduce hallucination, fresh/private knowledge without retraining); the full pipeline (ingest → chunk → embed → store → retrieve → rerank → augment prompt → generate → cite); indexing vs query time; the failure modes (bad retrieval dominates quality); RAG vs long-context vs fine-tuning decision; advanced variants (hybrid search, HyDE, query rewriting, parent-document, graph RAG, agentic RAG); evaluation (retrieval + generation); a worked reference architecture and Java/Python pseudocode.
