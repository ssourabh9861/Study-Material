...
**Subtopic:** Chunking & Retrieval

Cover at minimum: why chunking determines RAG quality; chunking strategies (fixed-size, overlap, recursive/structure-aware, semantic, sentence/paragraph, per-document-type) with tradeoffs; chunk size vs retrieval precision vs context cost; metadata enrichment; retrieval methods (dense, sparse/BM25, hybrid + fusion); top-k selection; the lost-context problem and parent-document/contextual retrieval; multi-vector and late-interaction (ColBERT) at a high level; evaluating retrieval (recall@k, MRR, nDCG); worked examples.
