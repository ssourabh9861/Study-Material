...
**Subtopic:** Reranking & Evaluation

Cover at minimum: the two-stage retrieve-then-rerank pattern and why it helps (cheap recall then precise ordering); cross-encoder rerankers vs bi-encoders (accuracy vs cost); reranker models (Cohere Rerank, BGE) and latency tradeoffs; RAG evaluation frameworks (RAGAS metrics — faithfulness, answer relevance, context precision/recall); LLM-as-judge (and its pitfalls/bias); building eval sets; offline vs online eval; regression testing prompts/pipelines; worked example of adding a reranker and measuring lift.
