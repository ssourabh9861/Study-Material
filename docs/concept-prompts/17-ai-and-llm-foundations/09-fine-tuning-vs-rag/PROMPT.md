...
Cover at minimum: what fine-tuning is (full vs parameter-efficient — LoRA/QLoRA explained); what fine-tuning is good for (style, format, narrow tasks, latency/cost reduction) vs what it's bad for (injecting fresh/factual knowledge); RAG vs fine-tuning vs prompt-engineering vs long-context — a clear decision framework (they're complementary, not either/or); data requirements and cost of fine-tuning; catastrophic forgetting; evaluation; when to combine RAG + a fine-tuned model; worked decision examples.
