...
**Subtopic:** Guardrails & LLM Observability

Cover at minimum: input/output guardrails (PII redaction, toxicity, topic/policy filters, schema validation, grounding checks); prompt-injection and jailbreak defenses (cross-reference agents); guardrail frameworks (NeMo Guardrails, Guardrails AI, Llama Guard); LLM observability (tracing prompts/completions/tokens/cost/latency, tools like LangSmith/Langfuse/Phoenix); evaluation in production (online metrics, user feedback, drift); cost and latency control (caching, routing to cheaper models, streaming); hallucination detection; worked instrumentation example.
