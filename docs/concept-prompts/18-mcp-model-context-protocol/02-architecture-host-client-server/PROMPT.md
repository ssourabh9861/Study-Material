...
**Subtopic:** Architecture — Host, Client, Server

Cover at minimum: the three roles — host (the LLM app, e.g. Claude Desktop/IDE), client (lives in the host, 1:1 with a server, manages the connection), server (exposes capabilities); the JSON-RPC 2.0 message types (requests, responses, notifications) and the lifecycle (initialize handshake, capability negotiation, operation, shutdown); capabilities advertisement; the trust boundary (host mediates, user consents); a diagram-in-text of the full topology; how this maps onto a real session.
