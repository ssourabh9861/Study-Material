...
**Subtopic:** Tools Primitive

Cover at minimum: tools as model-controlled actions (the model decides to call them); the tool definition (name, description, JSON Schema input — and why descriptions/schemas drive correct use); tools/list and tools/call methods; returning results (content types, isError); side-effecting vs read-only tools and annotations; designing good tools (granularity, idempotency, clear errors); human-in-the-loop approval; security (validate inputs, least privilege); worked example of defining and serving a tool.
