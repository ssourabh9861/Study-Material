...
**Subtopic:** Resources Primitive

Cover at minimum: resources as application-controlled context data (files, DB rows, API responses) exposed by URI; how resources differ from tools (data to read vs actions to perform; who controls inclusion — app/user vs model); resources/list, resources/read, resource templates (parameterized URIs); subscriptions and update notifications; MIME types and content; when to model something as a resource vs a tool; worked example exposing files/records as resources.
