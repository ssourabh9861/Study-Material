...
**Subtopic:** Building an MCP Server

Cover at minimum: an end-to-end build using an official SDK (TypeScript or Python — pick one and be concrete); project scaffolding, declaring capabilities, implementing tools/resources/prompts, wiring a transport (stdio first); testing with MCP Inspector; connecting it to a host (e.g. Claude Desktop config); error handling, logging, and lifecycle; packaging/distribution; a complete minimal but real server example with code; common mistakes.
