...
**Subtopic:** Planning & Decomposition

Cover at minimum: why decomposition helps (break a complex goal into subtasks); planning patterns — plan-and-execute (plan upfront, then run), ReWOO, tree-of-thoughts, hierarchical/orchestrator-worker; static vs dynamic re-planning; task graphs/DAGs of subtasks; the planner/executor split; handling failures and replanning mid-flight; cost implications of planning; when explicit planning beats letting the model improvise; worked example of a planning agent.
