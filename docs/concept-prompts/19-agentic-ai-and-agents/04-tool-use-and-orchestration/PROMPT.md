...
Cover at minimum: tools as the agent's hands (APIs, code execution, search, DB, MCP servers); designing tools for agents (clear names/descriptions, right granularity, good error messages the model can recover from, idempotency); the tool-calling loop and parallel tool calls; tool selection problems when there are many tools (retrieval over tools, namespacing); guarding side effects (dry-run, approval, sandboxing); code-execution/computer-use tools; how MCP fits (cross-reference); worked orchestration example.
