...
**Subtopic:** Agent Frameworks

Cover at minimum: the landscape and what each is for — LangChain/LangGraph (graph-based stateful orchestration, checkpointing), LlamaIndex (data/RAG-centric), CrewAI (role-based crews), Microsoft AutoGen/Semantic Kernel, OpenAI Agents SDK, Anthropic's tool-use/Claude Agent approach; build-vs-framework tradeoffs (frameworks add abstraction/lock-in; many production agents are hand-rolled loops); how they handle state/tools/memory/observability; choosing one (or none); a worked LangGraph-style example and the equivalent raw loop.
