...
Cover at minimum: why evaluating agents is hard (multi-step, non-deterministic, many valid paths); what to measure — task success/completion rate, trajectory/step quality, tool-call correctness, cost, latency, number of steps; outcome vs process evals; LLM-as-judge for agents and its pitfalls; building benchmark/eval suites and golden trajectories; agent benchmarks (SWE-bench, GAIA, tau-bench) at a high level; online eval and human feedback; regression testing agents in CI; observability/tracing as the basis for eval; worked eval setup.
