...
**Subtopic:** Production Agent Architecture

Cover at minimum: designing a production-grade agent/RAG-agent service end to end (the senior system-design version); the reference architecture (API/gateway, orchestration layer, model gateway/router, tool/MCP layer, memory + vector store, guardrails, observability/tracing, eval harness, human-in-the-loop); statefulness and resuming long-running agents; latency budgeting and streaming; cost control and model routing; reliability (retries, timeouts, fallbacks, idempotency for tool side effects); multi-tenancy and isolation; deployment/scaling; security; a worked design walkthrough as you would present in an interview.
