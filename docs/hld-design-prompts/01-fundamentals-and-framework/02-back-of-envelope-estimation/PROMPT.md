...
> NOTE: methodology/reference topic — adapt the structure toward a calculation playbook.

Cover at minimum: the numbers every engineer should know (latency numbers, QPS per server, storage per row, bandwidth, cache hit costs); how to estimate QPS, storage/5yr, bandwidth, memory, and server/shard counts from a product spec; read/write ratios and fan-out; peak vs average and headroom multipliers; powers-of-two and unit cheats; several fully worked estimations (newsfeed, video platform, chat); how to present estimates fast and confidently.
