...
**Topic:** Building Blocks Cheat-Sheet
> NOTE: reference topic — adapt the structure into a design-time catalog of building blocks.

Cover at minimum: each core building block as a *design-time decision* — load balancers (L4/L7), CDN, reverse proxy/API gateway, caches (where to place), message queues vs streams, SQL vs NoSQL, sharding, replication, search index, blob store, rate limiter, pub/sub, CDC; for each: what it solves, when to add it, its failure mode, and the one-line tradeoff; how they compose into a typical large-scale architecture; a reference diagram (Mermaid).
