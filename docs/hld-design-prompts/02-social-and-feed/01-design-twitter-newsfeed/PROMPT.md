...
Focus deep dives on: timeline generation — **fan-out-on-write vs fan-out-on-read** and the hybrid; the **celebrity/hot-key problem**; feed ranking; tweet storage and the social graph; pull vs push delivery; caching the timeline; handling media; eventual consistency of the feed; scale to hundreds of millions of users.
