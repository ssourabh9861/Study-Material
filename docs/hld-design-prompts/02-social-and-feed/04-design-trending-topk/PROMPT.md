...
**System:** Design Trending / Top-K

Focus deep dives on: computing top-K (trending hashtags/products) at scale in near-real-time; approximate algorithms (**Count-Min Sketch**, heavy hitters, **HyperLogLog** for cardinality — explain each); stream processing (windowing, tumbling/sliding); batch vs streaming (lambda/kappa); time-decay; handling hot keys and spam; serving the precomputed top-K with low latency.
