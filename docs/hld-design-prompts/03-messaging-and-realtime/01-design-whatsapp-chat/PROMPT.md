...
**System:** Design WhatsApp / a Chat System

Focus deep dives on: real-time delivery via **persistent connections (WebSocket) and the connection/presence layer**; message routing across gateway servers; 1:1 and group messaging; delivery receipts (sent/delivered/read) and ordering; offline message storage and sync; the message store schema; **end-to-end encryption** at a systems level; scaling connection servers (millions of concurrent sockets).
