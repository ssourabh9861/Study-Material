...
Focus deep dives on: channels/workspaces data model; real-time fan-out to many channel members; message history + search; presence and typing indicators; notifications across devices; unread counts per user per channel (a hard counting problem); threading; file sharing; multi-device sync.
