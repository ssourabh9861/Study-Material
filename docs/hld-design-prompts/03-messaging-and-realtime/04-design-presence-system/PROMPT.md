...
**System:** Design a Presence System (online/last-seen)

Focus deep dives on: tracking online/offline/last-seen for millions of users; heartbeat design and the failure-detection tradeoff (how fast to mark offline); storage (in-memory + TTL) and the write-amplification problem; fan-out of presence changes to subscribers (and why naive fan-out explodes); consistency vs cost; sharding the presence service.
