...
Focus deep dives on: low-latency ingest (RTMP) → transcode → segment → deliver; the latency vs scale tradeoff (HLS/LL-HLS vs WebRTC); CDN fan-out for millions of concurrent viewers; live chat alongside the stream; DVR/rewind; handling the thundering herd at stream start; transcoding cost.
