...
Focus deep dives on: real-time location ingestion from millions of drivers; **geospatial indexing** (geohash / S2 / quadtree / H3 — explain) for nearby-driver search; matching riders↔drivers (dispatch); ETA/pricing services; trip state machine; surge pricing; consistency of a ride lifecycle; the write-heavy location stream; sharding by geography.
