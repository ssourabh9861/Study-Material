...
**System:** Design an Ad Click Aggregator

Focus deep dives on: ingesting a massive click stream; real-time aggregation (windowed counts by ad/campaign) with stream processing; the batch+stream (lambda/kappa) reconciliation for accuracy; **deduplication** of clicks and fraud filtering; exactly-once-ish counting; time-series storage; serving aggregated metrics with low latency; hot keys.
