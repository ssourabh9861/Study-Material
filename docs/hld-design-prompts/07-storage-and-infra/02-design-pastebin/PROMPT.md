...
Focus deep dives on: storing large text blobs (blob store vs DB) and metadata split; key generation; read path + caching; expiration/TTL; access control (private/unlisted); size limits; analytics; scaling reads.
