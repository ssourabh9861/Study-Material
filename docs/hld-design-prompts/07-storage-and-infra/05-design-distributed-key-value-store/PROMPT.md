...
**System:** Design a Distributed Key-Value Store (Dynamo-style)

Focus deep dives on: partitioning via **consistent hashing**; replication (N/R/W quorums); tunable consistency; conflict resolution (vector clocks, LWW); anti-entropy (Merkle trees), hinted handoff; gossip membership; the CAP positioning; write/read path end to end; comparison to the building blocks the candidate knows.
