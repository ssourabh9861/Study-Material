...
**System:** Design a Distributed Job Scheduler / Task Queue

Focus deep dives on: scheduling (cron/delayed/one-off) and the **time-wheel / priority queue** design; exactly-once execution and idempotency; worker pool, leasing, and visibility timeout; retries + backoff + DLQ; handling worker failure (lease expiry); fairness across tenants; at-scale storage of jobs; preventing duplicate runs (distributed locks/leader election).
