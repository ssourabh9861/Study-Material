...
**LLD Problem:** Splitwise (expense sharing)
Likely patterns/focus: Strategy (split types: equal/exact/percent), Observer (balance updates), Factory (split creation). Entities: User, Group, Expense, Split, BalanceSheet/Ledger, settlement.
Extensions to address: simplify-debts/settle-up algorithm (minimize transactions), multi-currency, groups vs friends, partial payments, recurring expenses, comments/attachments, concurrency on shared balances.
