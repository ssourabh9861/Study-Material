...
Likely patterns/focus: State (elevator states), Strategy (scheduling/dispatch algorithm — SCAN/LOOK), Command (requests), Observer (displays). Entities: ElevatorSystem, ElevatorCar, Request (internal/external), Door, Direction, Scheduler.
Extensions to address: multiple elevators + optimal assignment, up/down external buttons, capacity/weight limits, express/zoned elevators, maintenance mode, concurrency of requests.
