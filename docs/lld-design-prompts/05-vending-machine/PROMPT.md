...
Likely patterns/focus: State pattern (Idle/HasMoney/Dispensing/SoldOut) is the centerpiece, Strategy (payment), Singleton (inventory). Entities: VendingMachine, State, Product, Inventory, Coin/Note, Dispenser.
Extensions to address: change computation (coin change), out-of-stock, refunds/cancel, multiple denominations, card payments, restocking, concurrency on inventory.
