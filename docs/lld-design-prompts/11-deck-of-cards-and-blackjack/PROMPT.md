...
**LLD Problem:** Deck of Cards & Blackjack
Likely patterns/focus: Factory (deck), Strategy (shuffle, hand-value/scoring), State (game rounds). Entities: Card, Deck, Shoe, Hand, Player, Dealer, Game.
Extensions to address: shuffling algorithm (Fisher-Yates), multiple decks/shoe, ace value logic, dealer rules, betting, split/double-down, extensibility to other card games.
