...
**LLD Problem:** Logging Framework
Likely patterns/focus: Chain of Responsibility (log levels/handlers), Strategy (formatters), Singleton (logger), Observer (appenders), Builder (config). Entities: Logger, LogLevel, LogMessage, Appender (console/file), Formatter, Filter.
Extensions to address: log levels & filtering, multiple appenders, async logging (queue + worker), thread-safety, rolling files, structured/JSON output, configurability.
