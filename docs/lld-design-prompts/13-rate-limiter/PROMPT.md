...
Likely patterns/focus: Strategy (algorithm: token bucket / leaky bucket / sliding window), Factory, thread-safety is central. Entities: RateLimiter interface, per-algorithm impls, ClientBucket, Clock.
Extensions to address: per-user/per-API keys, distributed enforcement (note Redis approach), thread-safe counters, configurable limits, fail-open vs fail-closed, multiple algorithms behind one interface.
