...
**LLD Problem:** In-Memory Key-Value Store (Redis-lite)
Likely patterns/focus: Command (operations), Strategy (eviction), Singleton (store), Observer (expiry). Entities: Store, Value types, ExpiryManager, TransactionManager.
Extensions to address: TTL/expiry (lazy + active), data types (string/list/hash), transactions (MULTI/EXEC), persistence hook, eviction policy, thread-safety, pub/sub.
