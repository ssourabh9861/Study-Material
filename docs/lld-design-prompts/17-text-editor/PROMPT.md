...
**LLD Problem:** Text Editor (with undo/redo)
Likely patterns/focus: Command (edits for undo/redo), Memento (snapshots), Strategy. Entities: Document, EditCommand hierarchy, History (undo/redo stacks), Cursor/Selection.
Extensions to address: undo/redo, cut/copy/paste, find/replace, efficient text buffer (gap buffer/rope — explain), cursor & selection, collaborative editing boundary.
