...
**LLD Problem:** Notification System
Likely patterns/focus: Observer (subscribers), Strategy (channel send), Factory (channel), Decorator (formatting), Builder (notification). Entities: NotificationService, Channel (email/SMS/push), User preferences, Template, RetryPolicy.
Extensions to address: multi-channel, user preferences/opt-out, templating, retries + DLQ, rate limiting per user, priority, async delivery, dedup/idempotency.
