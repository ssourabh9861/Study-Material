...
**LLD Problem:** Meeting Scheduler / Calendar
Likely patterns/focus: Strategy (conflict resolution/recurrence), Observer (invites/reminders), Factory. Entities: User, Calendar, Meeting, Room, Interval, RecurrenceRule, Scheduler.
Extensions to address: conflict detection (interval overlap), finding common free slots across attendees, room booking, recurring meetings, time zones, reminders/notifications, concurrency on room booking.
