...
**LLD Problem:** Restaurant Management System
Likely patterns/focus: State (order/table), Strategy (billing), Observer (kitchen display), Factory. Entities: Restaurant, Table, Reservation, Menu, Order, Bill, Waiter, Kitchen.
Extensions to address: table reservation & assignment, order lifecycle, split bills, menu management, waitlist, kitchen order queue, taxes/discounts, concurrency on tables.
