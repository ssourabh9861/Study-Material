...
**LLD Problem:** Traffic Signal Control System
Likely patterns/focus: State (signal colors), Observer (signal changes), Strategy (timing plan), Singleton (controller). Entities: TrafficController, Intersection, Signal, State (Red/Green/Yellow), Timer.
Extensions to address: timed vs sensor-based transitions, multi-direction intersections, emergency-vehicle override, configurable timing, synchronization across signals, concurrency/timers.
