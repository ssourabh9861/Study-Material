...
**LLD Problem:** Car Rental System
Likely patterns/focus: State (rental lifecycle), Strategy (pricing), Factory (vehicle), concurrency to prevent double-rental. Entities: Vehicle, VehicleType, Store/Branch, Reservation, Customer, Payment, Inventory.
Extensions to address: search by type/location/dates, availability over date ranges, double-booking prevention, pricing & insurance add-ons, pickup/drop branches, cancellations, late fees.
