...
**LLD Problem:** Chat Application (LLD)
Likely patterns/focus: Observer (message delivery), Mediator (chat room coordinates members), Strategy (delivery), Factory. Entities: User, ChatRoom/Group, Message, MessageQueue, Notification.
Extensions to address: 1:1 and group chat, delivery/read receipts, online status, message history, the Mediator pattern for group coordination, offline delivery, thread-safety.
