...
**LLD Problem:** Jukebox / Music Player
Likely patterns/focus: State (playing/paused/stopped), Strategy (play order: shuffle/repeat), Iterator (playlist), Observer (now-playing), Singleton. Entities: Player, Playlist, Song, Album, PlayStrategy, Queue.
Extensions to address: play/pause/next/prev, shuffle & repeat modes (Strategy), playlist management, queue, now-playing observers, song library/search.
