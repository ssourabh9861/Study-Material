...
**LLD Problem:** Task Management System (Jira/Trello)
Likely patterns/focus: State (task workflow), Observer (assignment/updates), Composite (subtasks), Strategy (sorting/filtering), Command (actions). Entities: User, Project, Task, Sprint/Board, Column, Comment, Label.
Extensions to address: task state workflow (todo→in-progress→done), assignment & notifications, subtasks (Composite), sprints/boards, priorities & labels, filtering/search, activity log, concurrency on updates.
