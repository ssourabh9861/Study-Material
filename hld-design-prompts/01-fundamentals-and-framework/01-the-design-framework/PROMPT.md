...
**Topic:** The HLD Interview Framework
> NOTE: This is a methodology/reference topic, not a single system. Adapt the structure: instead of designing one system, teach the repeatable approach.

Cover at minimum: a repeatable step-by-step framework for any system-design round (clarify requirements → estimate → define APIs → high-level design → data model → deep dives → bottlenecks → wrap-up); how to drive the requirements conversation (functional vs non-functional, the questions to always ask); how to manage time in a 45-min round; how to read interviewer signals and pivot; common ways candidates fail; how senior vs staff answers differ; a checklist you can run for any prompt.
