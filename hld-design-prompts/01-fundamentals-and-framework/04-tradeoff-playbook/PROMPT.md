...
**Topic:** Tradeoff Playbook
> NOTE: reference topic — adapt the structure into a catalog of recurring tradeoffs.

Cover at minimum: the recurring system-design tradeoffs with clear decision rules — SQL vs NoSQL, normalization vs denormalization, strong vs eventual consistency, sync vs async (and when to introduce a queue), push vs pull (fan-out-on-write vs read), monolith vs microservices, replication vs sharding, latency vs throughput vs cost, caching vs source-of-truth; for each, the signals that point to one side and the failure mode of the wrong choice; how to verbalize tradeoffs in an interview.
