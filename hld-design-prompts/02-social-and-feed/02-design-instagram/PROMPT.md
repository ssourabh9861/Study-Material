...
Focus deep dives on: photo/video upload + processing pipeline and blob storage + CDN; the feed (fan-out tradeoffs); the social graph (followers); stories (ephemeral, TTL); metadata store choice; thumbnail/transcoding; read-heavy scaling; consistency of likes/counts (distributed counters).
