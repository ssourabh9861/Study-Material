...
**System:** Design a Notification System

Focus deep dives on: multi-channel delivery (push/APNs-FCM, SMS, email, in-app); fan-out and templating; the queue-based pipeline and rate limiting/throttling per user; deduplication and idempotency; user preferences and quiet hours; retries + DLQ; delivery tracking; priority lanes; scaling to billions/day.
