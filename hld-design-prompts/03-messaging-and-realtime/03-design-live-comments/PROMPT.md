...
**System:** Design Live Comments / Live Chat (high fan-out)

Focus deep dives on: massive read fan-out (one event, millions of viewers); WebSocket/SSE at scale and connection sharding; pub/sub backbone; rate limiting and spam/abuse; eventual ordering vs strict ordering; backpressure when a stream is hotter than clients can consume; graceful degradation (sampling comments) under extreme load.
