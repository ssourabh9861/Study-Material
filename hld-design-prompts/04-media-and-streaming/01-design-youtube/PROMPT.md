...
Focus deep dives on: the **upload → transcode (multiple resolutions) → store → CDN** pipeline; chunked/resumable uploads; blob storage + metadata store split; adaptive bitrate streaming (HLS/DASH); view-count aggregation (distributed counter, hot videos); recommendations at a systems level; search; global CDN strategy; read-heavy scaling.
