...
Focus deep dives on: map tiling and serving; the routing/shortest-path problem at planet scale (graph partitioning, contraction hierarchies — high level); real-time traffic ingestion and incorporation into ETAs; geocoding/reverse-geocoding; place search; storage of road network; caching popular routes; scale.
