...
**System:** Design Hotel Booking (Booking.com)

Focus deep dives on: inventory/availability modeling; **preventing double-booking** (concurrency, holds/reservations with TTL, optimistic locking); search with filters + availability; pricing/rate plans; the booking transaction and payment; cancellation/refunds; calendar/date-range queries; consistency requirements; read vs write scaling.
