...
**System:** Design Ticketmaster / Event Booking

Focus deep dives on: seat inventory and **preventing double-booking under extreme contention** (seat holds with TTL, distributed locks vs optimistic concurrency, reservation queue); the flash-sale/thundering-herd problem (virtual waiting room); payment + confirmation saga; consistency (strong for seats); search; scaling reads vs the hot write path; fairness.
