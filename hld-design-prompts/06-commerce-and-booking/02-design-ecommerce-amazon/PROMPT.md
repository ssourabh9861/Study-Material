...
**System:** Design an E-commerce Platform (Amazon)

Focus deep dives on: catalog/search service; the cart and **inventory management** (reservation, oversell prevention); order placement saga across services; payment boundary; product reviews/ratings; recommendations boundary; read-heavy catalog scaling + caching; eventual vs strong consistency per subsystem; high availability on Black-Friday-scale traffic.
