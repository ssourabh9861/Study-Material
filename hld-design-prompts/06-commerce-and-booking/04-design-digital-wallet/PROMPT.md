...
**System:** Design a Digital Wallet

Focus deep dives on: account balances and the ledger; transactional correctness for transfers (atomic debit/credit, no double-spend) — **CAS/optimistic concurrency** vs locking; idempotent transactions; consistency and durability of money; handling concurrent updates to the same wallet; statements/history; scaling hot wallets; reconciliation.
