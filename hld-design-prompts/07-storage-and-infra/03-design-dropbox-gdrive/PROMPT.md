...
Focus deep dives on: file chunking + **content-addressable storage and dedup**; sync protocol (detecting changes, delta sync, conflict resolution); metadata service vs block storage; sharing/permissions; versioning; offline edits and conflict handling; large-file upload (resumable); consistency of the file tree.
