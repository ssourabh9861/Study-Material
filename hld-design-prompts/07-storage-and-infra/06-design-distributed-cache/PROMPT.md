...
**System:** Design a Distributed Cache

Focus deep dives on: partitioning keys (consistent hashing) across cache nodes; replication and failure handling; eviction (LRU/LFU) at scale; cache-aside integration; hot-key mitigation; client-side vs proxy topology (Redis Cluster vs twemproxy style); consistency with the source DB; cold-start/warming.
