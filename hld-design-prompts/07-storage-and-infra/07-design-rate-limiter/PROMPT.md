...
**System:** Design a Distributed Rate Limiter

Focus deep dives on: algorithm choice (token bucket vs sliding window) for distributed enforcement; where state lives (centralized Redis vs local+sync) and the accuracy-vs-latency tradeoff; race conditions and atomicity (Lua/atomic ops); per-user/global/tenant limits; failure mode (fail-open vs fail-closed); response design; scaling the limiter itself.
