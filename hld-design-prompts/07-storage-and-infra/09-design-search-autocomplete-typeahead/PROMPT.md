...
Focus deep dives on: the **trie**-based suggestion structure and how it's sharded/served; ranking suggestions (frequency, personalization); building/updating the trie from query logs (batch + streaming); prefix caching; latency budget (<100ms); handling typos; top-K per prefix precomputation; scale.
