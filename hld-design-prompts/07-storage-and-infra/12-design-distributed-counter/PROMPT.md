...
**System:** Design a Distributed Counter (likes/views)

Focus deep dives on: counting at massive write rates without contention; sharded/striped counters and aggregation; approximate vs exact; eventual consistency of the displayed count; hot-key handling (celebrity post); write batching/buffering; durability; read path; comparison of approaches (DB increment vs Redis vs CRDT counters).
