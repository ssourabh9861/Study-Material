...
**System:** Design a Log Collection & Analysis System

Focus deep dives on: high-throughput log ingestion (agents → buffer/Kafka → index); the indexing/search store (inverted index, ELK-style) and its cost; sampling and retention tiers; structured vs unstructured logs; querying at scale; backpressure when log volume spikes; multi-tenancy; cost control as the central tradeoff.
