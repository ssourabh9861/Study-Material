...
**LLD Problem:** Movie Ticket Booking (BookMyShow)
Likely patterns/focus: State (booking lifecycle), Strategy (pricing/seat-selection), Observer (seat availability), Factory; concurrency control to **prevent double-booking** (seat locking with TTL, optimistic locking). Entities: City, Cinema, Screen, Show, Seat, Booking, Payment.
Extensions to address: seat hold/expiry, concurrent booking of the same seat, dynamic pricing, cancellations/refunds, search by movie/city, multiple payment methods.
