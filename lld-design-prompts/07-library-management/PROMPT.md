...
Likely patterns/focus: Strategy (fine calculation/search), Observer (due/availability notifications), Factory. Entities: Book, BookItem (copies), Member, Librarian, Catalog, Lending/Reservation, Fine.
Extensions to address: reservations & holds queue, fines for late returns, search by author/title/subject, max-books limits, renewals, multiple copies, notifications.
