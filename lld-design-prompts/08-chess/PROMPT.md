...
**LLD Problem:** Chess
Likely patterns/focus: Strategy (per-piece movement rules), State (game state), Factory (piece creation), Command (moves for undo). Entities: Board, Cell, Piece hierarchy, Player, Move, Game.
Extensions to address: move validation per piece, check/checkmate/stalemate detection, castling/en-passant/promotion, undo/redo, turn management, draw conditions.
