...
**LLD Problem:** In-Memory File System
Likely patterns/focus: Composite (file/directory tree) is the centerpiece, Visitor (traversal/size), Singleton. Entities: Entry (abstract), File, Directory, FileSystem, Path resolver.
Extensions to address: nested directories, path operations (mkdir, ls, cd, find), file content read/write, permissions, symbolic links, search, in-memory vs persistence boundary.
