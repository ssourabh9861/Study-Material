...
**LLD Problem:** Food Delivery (Swiggy/Zomato)
Likely patterns/focus: State (order lifecycle), Strategy (delivery-partner assignment, pricing), Observer (order tracking), Factory. Entities: Customer, Restaurant, Menu, Order, DeliveryPartner, Cart, Payment.
Extensions to address: order state machine, partner assignment, live tracking, restaurant availability, surge/pricing, ratings, cancellations, concurrency on order/partner.
