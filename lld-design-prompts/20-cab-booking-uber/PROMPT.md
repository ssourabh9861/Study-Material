...
**LLD Problem:** Cab Booking (Uber/Ola)
Likely patterns/focus: State (trip lifecycle), Strategy (matching, pricing/surge), Observer (driver location/trip updates), Factory. Entities: Rider, Driver, Trip, Location, MatchingService, PricingStrategy.
Extensions to address: driver-rider matching (nearest), trip state machine, surge pricing, cancellations, ratings, ride types (pool/premium), concurrency on driver assignment.
