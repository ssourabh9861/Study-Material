...
**LLD Problem:** Hotel Booking / Reservation
Likely patterns/focus: State (reservation), Strategy (pricing/search), concurrency to **prevent double-booking**, Factory. Entities: Hotel, Room, RoomType, Reservation, Guest, Inventory, Payment.
Extensions to address: availability over date ranges, double-booking prevention (holds/optimistic locking), pricing/rate plans, cancellation policy, search & filters, overbooking strategy.
