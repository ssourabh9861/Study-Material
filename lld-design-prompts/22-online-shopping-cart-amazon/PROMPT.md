...
**LLD Problem:** Online Shopping Cart (Amazon)
Likely patterns/focus: Strategy (discount/pricing, payment), State (order), Composite (catalog categories), Observer (stock). Entities: Product, Catalog, Cart, CartItem, Order, Inventory, Payment, User.
Extensions to address: inventory/stock reservation, discounts/coupons (Strategy), multiple payment methods, order placement, wishlist, concurrency on stock, price calculation pipeline.
