...
**LLD Problem:** Payment Gateway / Processing
Likely patterns/focus: Strategy (payment method), State (payment lifecycle), Adapter (PSP integrations), Chain of Responsibility (fraud/validation), idempotency. Entities: PaymentService, PaymentMethod, Transaction, Gateway adapters, Ledger.
Extensions to address: multiple payment methods/PSPs, idempotency (no double charge), refunds, retries on async callbacks, fraud checks, status reconciliation, thread-safety of balance.
