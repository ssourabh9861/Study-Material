...
**LLD Problem:** Digital Wallet
Likely patterns/focus: State, Strategy (transaction types), optimistic concurrency (CAS) for balance, Command (transactions). Entities: Wallet, Account, Transaction, Ledger, TransferService.
Extensions to address: atomic transfers (debit+credit), no double-spend under concurrency, transaction history, add/withdraw money, idempotent transactions, multi-currency, reconciliation.
