...
**LLD Problem:** Stock Exchange / Order Matching Engine
Likely patterns/focus: Strategy (matching/order types), Observer (trade/quote feeds), State (order lifecycle), priority queues for the order book. Entities: Order, OrderBook (bids/asks), MatchingEngine, Trade, Account.
Extensions to address: limit vs market orders, price-time priority matching, partial fills, cancel/modify, the order-book data structure, concurrency/throughput, market data feed.
