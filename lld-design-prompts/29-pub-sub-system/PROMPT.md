...
**LLD Problem:** Pub-Sub System
Likely patterns/focus: Observer (core), Strategy (delivery), Singleton (broker), thread-safety. Entities: Broker, Topic, Publisher, Subscriber, Message, Subscription.
Extensions to address: topic-based routing, multiple subscribers, async delivery (queue + workers), at-least-once delivery & retries, thread-safe subscriber registry, backpressure, filtering.
