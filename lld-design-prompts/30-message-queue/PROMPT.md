...
**LLD Problem:** Message Queue (in-memory)
Likely patterns/focus: Producer-Consumer, Strategy (ordering/delivery), thread-safety with blocking queue, Observer. Entities: Queue, Producer, Consumer, Message, ConsumerGroup, OffsetTracker.
Extensions to address: thread-safe enqueue/dequeue (BlockingQueue), multiple consumers/groups, offsets & acknowledgements, at-least-once delivery, retries/DLQ, ordering guarantees, backpressure.
