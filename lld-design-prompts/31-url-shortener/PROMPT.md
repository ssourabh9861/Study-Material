...
**LLD Problem:** URL Shortener (LLD)
Likely patterns/focus: Strategy (key-generation: counter/base62 vs hash), Singleton (service), clean API. Entities: UrlService, Encoder/Decoder, Mapping store, ShortUrl.
Extensions to address: base62 encoding, collision handling, custom aliases, expiry, thread-safe ID generation, analytics counter, in-memory vs persistence boundary.
