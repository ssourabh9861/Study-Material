...
**LLD Problem:** Inventory Management System
Likely patterns/focus: Observer (low-stock alerts), Strategy (replenishment), State (stock), Singleton. Entities: Product, SKU, Warehouse, Stock, InventoryService, Supplier, Transaction.
Extensions to address: multi-warehouse stock, reservations/holds, low-stock alerts, replenishment, concurrency on stock updates (oversell prevention), audit trail, transfers.
